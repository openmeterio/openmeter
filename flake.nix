        ...
        devenv.shells = {
          default = {
            ...
            packages = with pkgs; [
              ...
              # $ nix-shell -p nix-prefetch-git jq --run "nix hash convert sha256:\$(nix-prefetch-git --url https://github.com/confluentinc/librdkafka.git --quiet --rev v2.14.0 | jq -r '.sha256')"
              (rdkafka.overrideAttrs (_: rec {
                src = fetchFromGitHub {
                  owner = "confluentinc";
                  repo = "librdkafka";
                  rev = "v2.14.0";
                  sha256 = "sha256-VXvjicBwwBTISiHwcLNWi1EKbLMd7Ks7EWf0Ds/q6B0=";
                };
              }))
