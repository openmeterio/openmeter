...
func TestSubscriptionCustomCurrencyBackfillAcrossPeriods(t *testing.T) {
		...
		Intents: charges.NewCreateChargeIntents(creditpurchase.Intent{
			...
			IntentMutableFields: creditpurchase.IntentMutableFields{
				IntentMutableFields: chargesmeta.IntentMutableFields{
					Name:              "Monthly advance backfill",
					ServicePeriod:     period,
					FullServicePeriod: period,
					BillingPeriod:     period,
				},
				CreditAmount: decimal.NewFromInt(15),
				EffectiveAt:  &at,
	...
	for _, status := range []payment.Status{payment.StatusAuthorized, payment.StatusSettled} {
		_, err = f.chargesService.HandleCreditPurchaseExternalPaymentStateTransition(ctx, charges.HandleCreditPurchaseExternalPaymentStateTransitionInput{
			ChargeID:           purchaseID,
	...
	expected := map[string]float64{
		chargeByMonth[time.April]: 10,
		chargeByMonth[time.May]:   5,
	}
	buckets, err := f.ledgerDeps.HistoricalLedger.GetBalanceBuckets(ctx, ledger.BalanceBucketQuery{
		...
		Filters: ledger.Filters{
			...
			Route: ledger.RouteFilter{
				...
				CostBasis:         mo.Some(lo.ToPtr(decimal.NewFromFloat(0.5))),
				CostBasisCurrency: mo.Some(lo.ToPtr(currencyx.Code("USD"))),
			},
		},
	...
	buckets, err = f.ledgerDeps.HistoricalLedger.GetBalanceBuckets(ctx, ledger.BalanceBucketQuery{
		...
		Filters: ledger.Filters{
			...
			Route: ledger.RouteFilter{
				Currency:                       f.currency.Reference(),
				CostBasis:                      mo.Some[*decimal.Decimal](nil),
