	...
	return &productcatalog.FlatFeeRateCard{
		RateCardMeta: productcatalog.RateCardMeta{
			...
			Price: productcatalog.NewPriceFrom(productcatalog.FlatPrice{
				Amount: alpacadecimal.NewFromInt(amount), PaymentTerm: productcatalog.InAdvancePaymentTerm,
			}),
		},
...
func createBilledComputeSubscription(t *testing.T, deps testDeps) (subscription.SubscriptionView, billing.StandardInvoice) {
	...
	input.ProRatingConfig = productcatalog.ProRatingConfig{Enabled: true, Mode: productcatalog.ProRatingModeProratePrices}
	...
	before, err := deps.subscriptionWorkflowService.CreateFromPlan(ctx, subscriptionworkflow.CreateSubscriptionWorkflowInput{
		Namespace: customer.Namespace, CustomerID: customer.ID,
		ChangeSubscriptionWorkflowInput: subscriptionworkflow.ChangeSubscriptionWorkflowInput{Timing: subscription.Timing{Enum: lo.ToPtr(subscription.TimingImmediate)}},
	}, p)
	require.NoError(t, err)
	...
	drafts, err := deps.billingService.InvoicePendingLines(ctx, billing.InvoicePendingLinesInput{Customer: customer.GetID(), AsOf: lo.ToPtr(clock.Now())})
	require.NoError(t, err)
...
func addComputeSizeToPlan(t *testing.T, deps testDeps, before subscription.SubscriptionView) subscription.Plan {
	t.Helper()
	original, err := deps.PlanService.GetPlan(t.Context(), plan.GetPlanInput{NamespacedID: models.NamespacedID{Namespace: before.Subscription.Namespace, ID: before.Subscription.PlanRef.Id}})
	require.NoError(t, err)
	...
	return deps.PlanHelper.CreatePlan(t, plan.CreatePlanInput{NamespacedModel: models.NamespacedModel{Namespace: original.Namespace}, Plan: target})
}
...
func migrateAndSyncSubscription(t *testing.T, deps testDeps, before subscription.SubscriptionView, target subscription.Plan) subscription.SubscriptionView {
	...
	response, err := deps.pcSubscriptionService.Migrate(t.Context(), plansubscription.MigrateSubscriptionRequest{
		ID: before.Subscription.NamespacedID, TargetVersion: lo.ToPtr(target.ToCreateSubscriptionPlanInput().Plan.Version),
	})
	require.NoError(t, err)
	for range 2 {
...
func assertIssuedInvoiceUnchanged(t *testing.T, deps testDeps, issued billing.StandardInvoice) {
	t.Helper()
	persisted, err := deps.billingService.GetStandardInvoiceById(t.Context(), billing.GetStandardInvoiceByIdInput{Invoice: issued.GetInvoiceID(), Expand: billing.StandardInvoiceExpandAll})
	require.NoError(t, err)
...
func assertOnlyAddedComputeNeedsBilling(t *testing.T, deps testDeps, after subscription.SubscriptionView) {
	...
	invoices, err := deps.billingService.ListGatheringInvoices(t.Context(), billing.ListGatheringInvoicesInput{
		Namespace: after.Subscription.Namespace, Customers: []string{after.Subscription.CustomerId},
		Page: pagination.Page{PageSize: 10, PageNumber: 1}, Expand: billing.GatheringInvoiceExpands{billing.GatheringInvoiceExpandLines},
	})
	require.NoError(t, err)
	...
	lines := lo.Filter(invoices.Items[0].Lines.OrEmpty(), func(line billing.GatheringLine, _ int) bool { return line.ServicePeriod.From.Before(monthEnd) })
