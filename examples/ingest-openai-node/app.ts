...
async function query() {
    ...
    const { data } = await openmeter.getValuesByMeterId('m2', 'my-awesome-user-id', from.toISOString(), to.toISOString(), WindowSize.DAY)
    console.log(data);
    ...
    const total = data.reduce((total, { value }) => total + (value || 0), 0)
