...
import { OpenMeter, WindowSize, Problem, MeterValue } from '@openmeter/sdk'
...
async function main() {
	...
	if (data.usage) {
		await openmeter.events.ingestEvents({
			specversion: '1.0',
...
async function query() {
	...
	const to = dayjs(from).add(1, 'day')
	const res = await openmeter.meters.getMeterValues(
		'm2',
		'my-awesome-user-id',
		from.toISOString(),
		to.toISOString(),
	...
	const data = res.data as MeterValue[]
