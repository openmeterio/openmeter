...
**SQL migration scripts embedded inline** — Guides requiring DB changes embed SQL directly in fenced sql blocks. Read-only SELECT audit queries come first; destructive UPDATE/DELETE statements come after. (`2025-06-26-subscription-alignment.md provides SELECT COUNT(*) audit queries before UPDATE plans SET billables_must_align = true.`)
**Before/After API examples for SDK migrations** — API-level changes show TypeScript before/after code blocks using the @openmeter/sdk client so SDK consumers can update integration code directly. (`2025-08-12 shows openmeter.subjects.createEntitlement → openmeter.customers.createEntitlement.`)
...
**Versioned event deprecation pattern** — Event schema breaking changes follow: introduce V(N+1), stop producing VN, drain VN (with an upgrade warning), then remove VN in a later release. (`2025-11-04-entitlement-events-v1.md: V2 introduced in commit 85f7ec90, V1 production stopped, V1 removed after drain confirmed.`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `2025-06-26-subscription-alignment.md` | Migration from unaligned to aligned subscriptions (billables_must_align flag). References openmeter/productcatalog/errors.go#L411 and alignment.go#L22 for the validation logic. | SQL targets plans and subscriptions tables directly — the billables_must_align column must still exist for these queries to work. |
| `2025-08-12-subject-customer-consolidation.md` | Consolidation of subject and customer entities — subjects deprecated as primary billing entities in favor of customers; explains automatic usage attribution by customer ID/Key. | usageAttribution.subjects is now optional; code assuming it is required needs updating. Subject APIs targeted for removal November 2025. |
| `2025-11-04-entitlement-events-v1.md` | Removal notice for V1 entitlement events after V2 replacement. Operators must drain V1 events before upgrading. | When adding new entitlement event versions, follow the same pattern: introduce V(N+1), stop producing VN, drain VN, then remove VN later. |
