...
Use standard library collection helpers first when they express the operation
directly, and use `github.com/samber/lo` for small, local collection
transformations when it makes the intent clearer than a hand-written loop. See
`go.mod` for the currently pinned version.
