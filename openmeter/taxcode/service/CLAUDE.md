...
**Key derivation convention for auto-created tax codes** — GetOrCreateByAppMapping derives the key as '<AppType>_<TaxCode>' when auto-creating from an app mapping. (`key := fmt.Sprintf("%s_%s", input.AppType, input.TaxCode)`)
...
| `taxcode.go` | All TaxCode CRUD plus GetOrCreateByAppMapping; get-then-mutate inside transaction.Run for atomicity. | The get-then-mutate pattern is safe only because the adapter's TransactingRepo rebinds to the same transaction. Breaking that rebinding makes the IsManagedBySystem guard non-atomic. |
| `organizationdefaulttaxcodes.go` | GetOrganizationDefaultTaxCodes and UpsertOrganizationDefaultTaxCodes; validates both referenced tax code IDs share a namespace before upsert. | Cross-namespace check is done via s.GetTaxCode (not the adapter directly) so namespace scoping is enforced. |
...
- Skipping the IsManagedBySystem check in new mutating methods — allows external callers to overwrite system-synced records
...
- Adding adapter dependencies beyond taxcode.Repository — the service must stay decoupled from Ent
