...
func (s *stateMachine) SyncFeatureIDFromFeatureMeter(ctx context.Context) error {
	if s.Charge.State.FeatureID != "" && !hasFeatureDependencyValidationIssue(s.Charge.ValidationIssues) {
		return nil
	}
	...
	s.Charge.ValidationIssues = withoutFeatureDependencyValidationIssues(s.Charge.ValidationIssues)
