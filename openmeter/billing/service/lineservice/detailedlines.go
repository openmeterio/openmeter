...
type getTotalAmountInput struct {
	...
	Discounts     billing.AmountLineDiscountsManaged
}
...
func mergeDetailedLines(line *billing.Line, in newDetailedLinesInput) error {
	...
	childrenWithIDReuse, err := line.Children.ChildrenWithIDReuse(detailedLines, line.ID)
	if err != nil {
...
func calculateDetailedLineTotals(line billing.DetailedLine) (billing.Totals, error) {
	...
	amount := calc.RoundToPrecision(line.FlatFee.PerUnitAmount.Mul(line.FlatFee.Quantity))

	switch line.FlatFee.Category {
