...
func (s *Service) UpdateTotalsFromDetailedLines(line *billing.Line) error {
	...
	for idx := range line.Children {
		detailedLine := &line.Children[idx]

		if detailedLine.IsDeleted() {
			continue
		}

		totals, err := calculateDetailedLineTotals(*detailedLine)
		if err != nil {
			return fmt.Errorf("calculating totals for line[%s]: %w", line.ID, err)
		}
		detailedLine.Totals = totals
	}
	...
	res = res.Add(lo.Map(line.Children, func(l billing.DetailedLine, _ int) billing.Totals {
