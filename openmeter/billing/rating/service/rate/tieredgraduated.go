...
type GraduatedTiered struct {
	ProgressiveBillingMeteredPricer
}

var _ Pricer = (*GraduatedTiered)(nil)

func (p GraduatedTiered) GenerateDetailedLines(l PricerCalculateInput) (rating.DetailedLines, error) {
	...
	out := make(rating.DetailedLines, 0, len(price.Tiers))

	err = p.TieredPriceCalculator(TieredPriceCalculatorInput{
		TieredPrice: price,
		FromQty:     usage.PreLinePeriodQuantity,
		ToQty:       usage.Quantity.Add(usage.PreLinePeriodQuantity),
		Currency:    l.CurrencyCalculator,
		TierCallbackFn: func(in TierCallbackInput) error {
			tierIndex := in.TierIndex + 1

			if in.Tier.UnitPrice != nil {
				newLine := rating.DetailedLine{
					Name:                   fmt.Sprintf("%s: usage price for tier %d", l.GetName(), tierIndex),
					Quantity:               in.Quantity,
					PerUnitAmount:          in.Tier.UnitPrice.Amount,
					ChildUniqueReferenceID: fmt.Sprintf(rating.GraduatedTieredPriceUsageChildUniqueReferenceID, tierIndex),
			...
			if in.Tier.FlatPrice != nil && in.AtTierBoundary && shouldFirstFlatLineBeBilled {
				newLine := rating.DetailedLine{
					Name:                   fmt.Sprintf("%s: flat price for tier %d", l.GetName(), tierIndex),
					Quantity:               alpacadecimal.NewFromFloat(1),
					PerUnitAmount:          in.Tier.FlatPrice.Amount,
					ChildUniqueReferenceID: fmt.Sprintf(rating.GraduatedTieredFlatPriceChildUniqueReferenceID, tierIndex),
					PaymentTerm:            productcatalog.InArrearsPaymentTerm,
				}
...
func (p GraduatedTiered) splitTierRangeAtBoundary(from, to alpacadecimal.Decimal, qtyRange TierRange) []TierRange {
...
func (p GraduatedTiered) TieredPriceCalculator(in TieredPriceCalculatorInput) error {
