...
func (p GraduatedTiered) GenerateDetailedLines(l PricerCalculateInput) (pricer.DetailedLines, error) {
	...
	out := make(pricer.DetailedLines, 0, len(price.Tiers))
	...
	err = p.TieredPriceCalculator(TieredPriceCalculatorInput{
		...
		TierCallbackFn: func(in TierCallbackInput) error {
			...
			if in.Tier.UnitPrice != nil {
				newLine := pricer.DetailedLine{
					Name:                   fmt.Sprintf("%s: usage price for tier %d", l.GetName(), tierIndex),
					Quantity:               in.Quantity,
					PerUnitAmount:          in.Tier.UnitPrice.Amount,
					ChildUniqueReferenceID: fmt.Sprintf(pricer.GraduatedTieredPriceUsageChildUniqueReferenceID, tierIndex),
			...
			if in.Tier.FlatPrice != nil && in.AtTierBoundary && shouldFirstFlatLineBeBilled {
				newLine := pricer.DetailedLine{
					...
					ChildUniqueReferenceID: fmt.Sprintf(pricer.GraduatedTieredFlatPriceChildUniqueReferenceID, tierIndex),
					PaymentTerm:            productcatalog.InArrearsPaymentTerm,
				}
