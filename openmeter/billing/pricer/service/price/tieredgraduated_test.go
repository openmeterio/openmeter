...
func TestTieredPriceCalculator(t *testing.T) {
	...
	graduatedTiered := price.GraduatedTiered{}
	...
	t.Run("tier callback, mid tier invocation", func(t *testing.T) {
		...
		callback.On("TierCallbackFn", price.TierCallbackInput{
			Tier:      testIn.Tiers[0],
			TierIndex: 0,
		...
		require.NoError(t, graduatedTiered.TieredPriceCalculator(
			price.TieredPriceCalculatorInput{
				TieredPrice: testIn,
...
func getTotalAmountForGraduatedTieredPrice(t *testing.T, qty alpacadecimal.Decimal, tieredPrice productcatalog.TieredPrice) alpacadecimal.Decimal {
	...
	pricer := price.GraduatedTiered{}

	total := alpacadecimal.Zero
	err := pricer.TieredPriceCalculator(price.TieredPriceCalculatorInput{
...
func introspectTieredPriceRangesFn(t *testing.T) func([]price.TierRange) {
	return func(qtyRanges []price.TierRange) {
...
func (m *mockableTieredPriceCalculator) TierCallbackFn(i price.TierCallbackInput) error {
	args := m.Called(i)
	return args.Error(0)
}
