...
func (m *MinAmountCommitment) Mutate(i price.PricerCalculateInput, pricerResult pricer.DetailedLines) (pricer.DetailedLines, error) {
	...
	if totalBilledAmount.LessThan(*commitments.MinimumAmount) {
		...
		pricerResult = append(pricerResult, pricer.DetailedLine{
...
func (m *MaxAmountCommitment) Mutate(i price.PricerCalculateInput, pricerResult pricer.DetailedLines) (pricer.DetailedLines, error) {
	commitments := i.GetPrice().GetCommitments()
