...
func (r *Reconciler) Reconcile(ctx context.Context, prices []llmcost.SourcePrice) error {
	...
	for key, sourcePrices := range groups {
		...
		if agreeing == nil {
			r.logger.DebugContext(ctx, "no price agreement",
		...
		if err := r.repo.UpsertGlobalPrice(ctx, price); err != nil {
			r.logger.ErrorContext(ctx, "failed to upsert global price",
				"provider", key.Provider,
				"model_id", key.ModelID,
				"error", err)

			upsertErrs = append(upsertErrs, fmt.Errorf("%s/%s: %w", key.Provider, key.ModelID, err))
...
func pricesAgree(a, b llmcost.ModelPricing, tolerance alpacadecimal.Decimal) bool {
	return decimalsAgree(a.InputPerToken, b.InputPerToken, tolerance) &&
		...
		optionalDecimalsAgree(a.CacheReadPerToken, b.CacheReadPerToken, tolerance) &&
		optionalDecimalsAgree(a.CacheWritePerToken, b.CacheWritePerToken, tolerance) &&
		optionalDecimalsAgree(a.ReasoningPerToken, b.ReasoningPerToken, tolerance)
}

// optionalDecimalsAgree checks if two optional decimals agree.
// Both nil = agree, one nil + one non-nil = disagree, both non-nil = delegate to decimalsAgree.
func optionalDecimalsAgree(a, b *alpacadecimal.Decimal, tolerance alpacadecimal.Decimal) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
...
func averagePrices(prices []llmcost.SourcePrice) (llmcost.SourcePrice, error) {
	...
	result.Pricing.CacheReadPerToken = averageOptionalDecimal(prices, count, func(p llmcost.SourcePrice) *alpacadecimal.Decimal { return p.Pricing.CacheReadPerToken })
	result.Pricing.CacheWritePerToken = averageOptionalDecimal(prices, count, func(p llmcost.SourcePrice) *alpacadecimal.Decimal { return p.Pricing.CacheWritePerToken })
	result.Pricing.ReasoningPerToken = averageOptionalDecimal(prices, count, func(p llmcost.SourcePrice) *alpacadecimal.Decimal { return p.Pricing.ReasoningPerToken })

	return result, nil
}

// averageOptionalDecimal averages an optional decimal field across prices.
// Returns nil if the first price has the field unset (agreement ensures consistency).
func averageOptionalDecimal(prices []llmcost.SourcePrice, count alpacadecimal.Decimal, getter func(llmcost.SourcePrice) *alpacadecimal.Decimal) *alpacadecimal.Decimal {
	if getter(prices[0]) == nil {
		return nil
	}

	sum := alpacadecimal.NewFromInt(0)
	for _, p := range prices {
		if v := getter(p); v != nil {
			sum = sum.Add(*v)
		}
