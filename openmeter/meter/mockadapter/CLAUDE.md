...
**Two-level struct: adapter (Service) + manageAdapter (ManageService)** — adapter implements meter.Service with an in-memory slice; manageAdapter embeds adapter and adds Create/Update/Delete/RegisterPreUpdateMeterHook, satisfying meter.ManageService. (`type manageAdapter struct { adapter *adapter; meter.Service }`)
**New vs NewManage factory selection** — Tests needing read-only access call New(meters); tests needing full mutation access call NewManage(meters). Both validate input meters. (`func NewManage(meters []meter.Meter) (meter.ManageService, error)`)
**Optional PG sync via SetDBClient** — Call adapter.SetDBClient(*entdb.Client) after New to make CreateMeter/ReplaceMeters persist rows to PostgreSQL for FK integrity. PG sync happens before in-memory state updates. (`adapter.SetDBClient(client) // enables FK-safe test scenarios`)
...
| `manage.go` | In-memory Create/Update/Delete implementations. RegisterPreUpdateMeterHook is a noop. | UpdateMeter matches by ID or Key in the in-memory slice — both must match the namespace to avoid cross-namespace mutations. |
...
- Expecting RegisterPreUpdateMeterHook to work — it is a noop in the mock.
- Using context.Background() in tests when t.Context() is available — breaks test-scoped cancellation.
