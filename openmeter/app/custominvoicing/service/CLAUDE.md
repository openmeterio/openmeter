...
**transaction.Run wrapping multi-step writes** — Operations touching multiple entities run inside transaction.Run or transaction.RunWithNoValue for atomicity. (`return transaction.Run(ctx, s.adapter, func(ctx context.Context) (app.AppBase, error) { appBase, _ := s.appService.CreateApp(...); s.adapter.UpsertAppConfiguration(...); return appBase, nil })`)
**Config struct with Validate() + compile-time assertion** — Service constructor takes a Config struct; Validate() checks all required fields. var _ appcustominvoicing.Service = (*Service)(nil) enforces the interface at compile time. (`var _ appcustominvoicing.Service = (*Service)(nil)`)
...
| File | Role | Watch For |
|------|------|-----------|
| `service.go` | Service struct, Config, Validate, New constructor; sets up the three dependencies: adapter, appService, billingService. | Service struct is exported (uppercase) unlike most sibling packages — intentional to allow direct *Service injection in Wire provider sets. |
...
| `factory.go` | FactoryService implementation: CreateApp (appService.CreateApp + adapter.UpsertAppConfiguration), DeleteApp, UpsertAppConfiguration, GetAppConfiguration. | CreateApp wraps both steps in a single transaction.Run — if adapter.UpsertAppConfiguration fails, the app row is rolled back. |
...
- **Critical-issue check causes transaction rollback in HandlePaymentTrigger** — Ensures the state-machine trigger and any side-effects are atomically reverted if the invoice ends up in an invalid state.

## Example: Add a new SyncService method that fires a billing trigger and validates post-trigger state
...
func (s *Service) VoidInvoice(ctx context.Context, input appcustominvoicing.VoidInvoiceInput) (billing.StandardInvoice, error) {
    ...
    return transaction.Run(ctx, s.adapter, func(ctx context.Context) (billing.StandardInvoice, error) {
        ...
        if err != nil { return billing.StandardInvoice{}, err }
        return s.billingService.GetStandardInvoiceById(ctx, billing.GetStandardInvoiceByIdInput{Invoice: input.InvoiceID})
    })
}
```
