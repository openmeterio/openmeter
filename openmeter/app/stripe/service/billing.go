...
func (s *Service) HandleInvoiceStateTransition(ctx context.Context, input appstripeentity.HandleInvoiceStateTransitionInput) error {
	...
	ctx = context.WithValue(ctx, "stripe_invoice_id", input.Invoice.ID)
	...
	ctx = context.WithValue(ctx, "invoice_id", invoice.ID)
	ctx = context.WithValue(ctx, "invoice_status", invoice.Status)

	if input.TargetStatus == invoice.Status {
	...
	if input.GetValidationErrors != nil {
		...
		if stripeValidationErrors != nil {
			component, err := s.getInvoicingComponentName(ctx, input.AppID, stripeValidationErrors.Op)
			if err != nil {
				return err
			}
			...
			validationErrors = &billing.InvoiceTriggerValidationInput{
				Component: component,
				ValidationErrors: lo.Map(stripeValidationErrors.Errors, func(stripeErr *stripe.Error, _ int) error {
	...
	err = s.billingService.TriggerInvoice(ctx, billing.InvoiceTriggerInput{
...
func (s *Service) getInvoicingComponentName(ctx context.Context, appID appentitybase.AppID, op billing.InvoiceOperation) (billing.ComponentName, error) {
	app, err := s.appService.GetApp(ctx, appID)
	if err != nil {
		return "", err
	}

	return billing.AppTypeCapabilityToComponent(
		app.GetType(),
