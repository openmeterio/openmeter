...
func (f *fakeCustomerService) ListCustomers(_ context.Context, input customer.ListCustomersInput) (pagination.Result[customer.Customer], error) {
	// Delegate to the production input validation so the tests exercise the
	// same filter rules the real service enforces (e.g. pkg/filter rejects
	// empty filter values).
	if err := input.Validate(); err != nil {
		return pagination.Result[customer.Customer]{}, err
	}
...
func TestListSubjects(t *testing.T) {
	...
	t.Run("EmptySubjectKeyDoesNotReachCustomerFilters", func(t *testing.T) {
		// given:
		// - a streaming batch containing an empty subject key, as written by a
		//   direct producer bypassing the API
		// when:
		// - listing with the attributed filter, which builds customer filters
		//   from the batch keys (pkg/filter rejects empty values)
		// then:
		// - the listing succeeds and the empty key is not attributed
		a, _ := newAdapter([]string{"", "s1", "s2"}, map[string]string{"s2": "customer-2"})
	...
	t.Run("AttributedFilterExactlyFullPageStopsScanning", func(t *testing.T) {
		// given:
		// - 250 subjects in batches of 100, every tenth one attributed, so a
		//   limit-10 page fills exactly when the first batch is consumed
		keys := make([]string, 0, 250)
		attribution := map[string]string{}
		for i := range 250 {
			key := fmt.Sprintf("s%03d", i)
		...
		// when:
		// - listing attributed subjects with a limit matching the per-batch match count
		// then:
		// - the scan stops after one batch with a continuation cursor at the
		//   last scanned key instead of prefetching the next batch
		result, err := a.ListSubjects(t.Context(), meterevent.ListSubjectsParams{
			Namespace:  "ns",
			Limit:      lo.ToPtr(10),
			Attributed: lo.ToPtr(true),
		...
		require.Equal(t, 1, connector.calls)
		require.NotNil(t, result.NextCursor)
		require.Equal(t, "s099", result.NextCursor.ID)
	})
