...
func TestOverrideCustomerEntitlementInputValidate(t *testing.T) {
	valid := OverrideCustomerEntitlementInput{
		CustomerID:    customer.CustomerID{Namespace: "ns", ID: "01K4WAQ0J99ZZ0MD75HXR112H8"},
		EntitlementID: "01K4WAQ0J99ZZ0MD75HXR112HA",
		Entitlement: CreateEntitlementInputs{
			FeatureID:       lo.ToPtr("01K4WAQ0J99ZZ0MD75HXR112H9"),
			EntitlementType: EntitlementTypeMetered,
		},
...
func TestGetEntitlementByIDInputValidate(t *testing.T) {
	valid := GetEntitlementByIDInput{
		Namespace:     "ns",
		EntitlementID: "01K4WAQ0J99ZZ0MD75HXR112H9",
	}
...
func TestListNamespaceEntitlementsInputValidate(t *testing.T) {
	valid := ListNamespaceEntitlementsInput{
		Namespace:  "ns",
		CustomerID: &filter.FilterULID{In: &[]string{"01K4WAQ0J99ZZ0MD75HXR112H8"}},
		FeatureID:  &filter.FilterULID{Eq: lo.ToPtr("01K4WAQ0J99ZZ0MD75HXR112H9")},
		FeatureKey: &filter.FilterString{In: &[]string{"a", "b"}},
		Type:       &filter.FilterString{Eq: lo.ToPtr(string(EntitlementTypeMetered))},
		OrderBy:    ListEntitlementsOrderByUpdatedAt,
	...
	t.Run("rejects a malformed customer id filter", func(t *testing.T) {
		input := valid
		input.CustomerID = &filter.FilterULID{Eq: lo.ToPtr("not-a-ulid")}

		require.ErrorContains(t, input.Validate(), "customer ID filter")
	})
