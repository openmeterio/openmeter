...
func TestQueryMeter(t *testing.T) {
	...
	tests := []struct {
		...
		{ // Aggregate all available data
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ?",
		...
		{ // Aggregate with count aggregation
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), count(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ?",
			wantArgs: []interface{}{"my_namespace", "meter1"},
		},
		{ // Aggregate data from start
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND om_events.time >= ?",
		...
		{ // Aggregate data between period
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND om_events.time >= ? AND om_events.time <= ?",
		...
		{ // Aggregate data for a single subject
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value, om_events.subject FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND (om_events.subject = ?) GROUP BY subject",
		...
		{ // Aggregate data for a multiple subjects
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value, om_events.subject FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND (om_events.subject = ? OR om_events.subject = ?) GROUP BY subject",
		...
		{ // Aggregate data with filtering for a single group and single value
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND (JSON_VALUE(om_events.data, '$.group1') = 'g1v1')",
			wantArgs: []interface{}{"my_namespace", "meter1"},
		},
		{ // Aggregate data with filtering for a single group and multiple values
			...
			wantSQL:  "SELECT min(windowstart), max(windowend), sum(cast(JSON_VALUE(om_events.data, '$.value'), 'Float64')) AS value FROM openmeter.om_events WHERE om_events.namespace = ? AND om_events.meter = ? AND (JSON_VALUE(om_events.data, '$.group1') = 'g1v1' OR JSON_VALUE(om_events.data, '$.group1') = 'g1v2')",
			wantArgs: []interface{}{"my_namespace", "meter1"},
		},
