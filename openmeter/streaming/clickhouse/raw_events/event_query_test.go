...
func TestQueryEventsTable(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  "SELECT id, type, subject, source, time, data, validation_error, ingested_at, stored_at FROM openmeter.om_events WHERE namespace = ? AND time >= ? ORDER BY time DESC LIMIT 100",
		...
		},
		{
			...
			wantSQL:  "SELECT id, type, subject, source, time, data, validation_error, ingested_at, stored_at FROM openmeter.om_events WHERE namespace = ? AND time >= ? AND subject = ? ORDER BY time DESC LIMIT 100",
			wantArgs: []interface{}{"my_namespace", from.Unix(), subjectFilter},
		},
		{
			query: queryEventsTable{
				Database:        "openmeter",
			...
			wantSQL:  "SELECT id, type, subject, source, time, data, validation_error, ingested_at, stored_at FROM openmeter.om_events WHERE namespace = ? AND time >= ? AND id LIKE ? ORDER BY time DESC LIMIT 100",
			wantArgs: []interface{}{"my_namespace", from.Unix(), "%event-id-1%"},
		},
		{
			query: queryEventsTable{
				Database:        "openmeter",
			...
			wantSQL:  "SELECT id, type, subject, source, time, data, validation_error, ingested_at, stored_at FROM openmeter.om_events WHERE namespace = ? AND time >= ? AND notEmpty(validation_error) = 1 ORDER BY time DESC LIMIT 100",
			wantArgs: []interface{}{"my_namespace", from.Unix()},
		},
		{
			query: queryEventsTable{
			...
			wantSQL:  "SELECT id, type, subject, source, time, data, validation_error, ingested_at, stored_at FROM openmeter.om_events WHERE namespace = ? AND time >= ? AND empty(validation_error) = 1 ORDER BY time DESC LIMIT 100",
			wantArgs: []interface{}{"my_namespace", from.Unix()},
		},
	}
