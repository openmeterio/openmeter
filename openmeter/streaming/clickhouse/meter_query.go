...
func (d *queryMeter) toSQL() (string, []interface{}, error) {
	...
	default:
		...
		selectColumns = append(selectColumns, fmt.Sprintf("%s(toFloat64OrNull(ifNotFinite(JSON_VALUE(%s, '%s'), null))) AS value", sqlAggregation, getColumn("data"), sqlbuilder.Escape(*d.Meter.ValueProperty)))
	}
