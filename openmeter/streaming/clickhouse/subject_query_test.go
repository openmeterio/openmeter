...
func TestListSubjectsV2(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantSQL:  "SELECT subject FROM openmeter.om_events WHERE namespace = ? AND subject <> ? AND subject ILIKE ? GROUP BY namespace, subject ORDER BY namespace, subject LIMIT ?",
