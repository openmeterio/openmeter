...
func TestQueryEventsTable(t *testing.T) {
	...
	tests := []struct {
		...
		// Customer filter
		{
			...
			wantSQL: "SELECT id, type, subject, source, time, data, ingested_at, stored_at, store_row_id, mapFromArrays(?, ?)[om_events.subject] AS customer_id FROM openmeter.om_events WHERE namespace = ? AND time >= ? AND om_events.subject IN (?) ORDER BY time DESC LIMIT ?",
			wantArgs: []interface{}{
				[]string{"subject1", "subject2", "subject3"},
				[]string{"customer1", "customer1", "customer2"},
				"my_namespace",
				from.Unix(),
				[]string{"subject1", "subject2", "subject3"},
				100,
