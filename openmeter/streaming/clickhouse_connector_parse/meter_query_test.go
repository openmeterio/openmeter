...
func TestQueryMeter(t *testing.T) {
	...
	tests := []struct {
		...
		{ // Aggregate all available data
			...
			wantSQL:  "SELECT tumbleStart(min(om_meter_events.time), toIntervalMinute(1)) AS windowstart, tumbleEnd(max(om_meter_events.time), toIntervalMinute(1)) AS windowend, sum(om_meter_events.value) AS value FROM openmeter.om_meter_events WHERE om_meter_events.namespace = ? AND om_meter_events.meter = ?",
		...
		{ // Aggregate data from start
			...
			wantSQL:  "SELECT tumbleStart(min(om_meter_events.time), toIntervalMinute(1)) AS windowstart, tumbleEnd(max(om_meter_events.time), toIntervalMinute(1)) AS windowend, sum(om_meter_events.value) AS value FROM openmeter.om_meter_events WHERE om_meter_events.namespace = ? AND om_meter_events.meter = ? AND om_meter_events.time >= ?",
		...
		{ // Aggregate data between period
			...
			wantSQL:  "SELECT tumbleStart(min(om_meter_events.time), toIntervalMinute(1)) AS windowstart, tumbleEnd(max(om_meter_events.time), toIntervalMinute(1)) AS windowend, sum(om_meter_events.value) AS value FROM openmeter.om_meter_events WHERE om_meter_events.namespace = ? AND om_meter_events.meter = ? AND om_meter_events.time >= ? AND om_meter_events.time <= ?",
		...
		{ // Aggregate data with filtering for a single group and single value
			...
			wantSQL:  "SELECT tumbleStart(min(om_meter_events.time), toIntervalMinute(1)) AS windowstart, tumbleEnd(max(om_meter_events.time), toIntervalMinute(1)) AS windowend, sum(om_meter_events.value) AS value FROM openmeter.om_meter_events WHERE om_meter_events.namespace = ? AND om_meter_events.meter = ? AND (om_meter_events.group_by['g1'] = ?)",
