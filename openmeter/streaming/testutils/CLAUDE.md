...
**SimpleEvent + AddSimpleEvent for usage-based test setup** — Register per-meter events with AddSimpleEvent(meterSlug, value, time, opts...). Use WithStoredAt(t) to set a StoredAt different from the event Time, exercising stored-at cutoff logic in billing finalisation. (`conn.AddSimpleEvent("api-calls", 10, now, testutils.WithStoredAt(now.Add(-time.Minute)))`)
**AddRow for exact pre-canned QueryMeter results** — When the approximate aggregation is insufficient, use AddRow(meterSlug, meter.MeterQueryRow{...}) to register exact results. QueryMeter returns rows whose WindowStart/WindowEnd match params.From/To exactly. (`conn.AddRow("meter-key", meter.MeterQueryRow{WindowStart: from, WindowEnd: to, Value: 42.0, GroupBy: map[string]*string{}})`)
**Reset() between subtests** — Call conn.Reset() to clear all rows and events between test cases that share a connector instance. (`conn.Reset()`)
**filterStoredAt mirrors Unix-second ClickHouse precision** — The mock's filterStoredAt evaluates FilterTimeUnix using Unix() seconds, matching ClickHouse's DateTime column precision. Tests relying on sub-second StoredAt distinctions will not work correctly. (`// Use full-second boundaries for StoredAt filters in tests`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `streaming.go` | Entire package implementation: MockStreamingConnector, SimpleEvent, AddSimpleEvent, SetSimpleEvents, AddRow, QueryMeter aggregation logic, filterStoredAt helper. | aggregateEvents returns MeterNotFoundError if the meterSlug has never been registered via AddSimpleEvent or AddRow — call AddSimpleEvent at least once or use SetSimpleEvents(slug, func(e []SimpleEvent) []SimpleEvent { return e }) to register an empty slice. |
...
- Using MockStreamingConnector for tests that need real ClickHouse SQL behaviour (e.g. PREWHERE, decimal precision edge cases) — use ConnectorTestSuite with TEST_CLICKHOUSE_DSN instead.
