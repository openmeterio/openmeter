...
func TestBalanceBucketsQuery_SQLUngroupedDimensions(t *testing.T) {
	...
	require.Equal(t, `WITH "balance_buckets" AS (SELECT "ledger_entries"."sub_account_id", NULL AS "source_charge_id", NULL AS "spend_charge_id", NULL AS "origin_id", SUM("ledger_entries"."amount") AS "sum_amount" FROM "ledger_entries" WHERE "ledger_entries"."namespace" = $1 GROUP BY "ledger_entries"."sub_account_id") SELECT "balance_buckets"."sub_account_id", "balance_buckets"."source_charge_id", "balance_buckets"."spend_charge_id", "balance_buckets"."origin_id", "balance_buckets"."sum_amount", "sub_accounts"."route_id", "accounts"."account_type", "routes"."routing_key_version", "routes"."routing_key", "routes"."currency", "routes"."cost_basis_currency", "routes"."tax_code", "routes"."tax_behavior", "routes"."features", "routes"."cost_basis", "routes"."credit_priority", "routes"."transaction_authorization_status" FROM "balance_buckets" JOIN "ledger_sub_accounts" AS "sub_accounts" ON "balance_buckets"."sub_account_id" = "sub_accounts"."id" JOIN "ledger_accounts" AS "accounts" ON "sub_accounts"."account_id" = "accounts"."id" JOIN "ledger_sub_account_routes" AS "routes" ON "sub_accounts"."route_id" = "routes"."id" ORDER BY "balance_buckets"."sub_account_id", "balance_buckets"."source_charge_id", "balance_buckets"."spend_charge_id", "balance_buckets"."origin_id"`, sqlStr)
