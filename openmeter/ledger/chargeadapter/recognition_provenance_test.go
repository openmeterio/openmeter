...
func TestCustomCurrencyRecognizedCorrectionPreservesSpend(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:    "same cost basis",
			sources: 1,
		},
		{
			name:           "different cost bases",
			differentBasis: true,
			sources:        1,
		},
		{
			name:    "multiple sources per allocation",
			sources: 2,
		},
	} {
				...
				for range tc.sources {
					...
					inputs, err := transactions.ResolveTransactions(
						...
						transactions.IssueCustomerReceivableTemplate{
							At:                env.Now(),
							Amount:            amount.Div(alpacadecimal.NewFromInt(int64(tc.sources))),
							Currency:          custom.Reference(),
							CostBasis:         &basis,
							CostBasisCurrency: &fiat,
							SourceChargeID:    &sourceID,
						},
					)
			...
			for _, corrected := range []int64{10, 20} {
				corrections, err := env.handler.OnCorrectCreditAllocations(t.Context(), chargeflatfee.CorrectCreditAllocationsInput{
					...
					Corrections: creditrealization.CorrectionRequest{{
						Allocation: charge.Realizations.CurrentRun.CreditRealizations[0],
						Amount:     alpacadecimal.NewFromInt(-corrected),
					}},
				...
				correctionGroup, err := env.Deps.HistoricalLedger.GetTransactionGroup(t.Context(), models.NamespacedID{
					Namespace: env.Namespace,
					ID:        corrections[0].LedgerTransaction.TransactionGroupID,
				})
				require.NoError(t, err)
