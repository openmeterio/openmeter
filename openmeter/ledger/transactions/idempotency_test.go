...
func TestCommitGroupIdempotencyCustomCurrencyReplayStability(t *testing.T) {
	// given:
	// - one keyed issuance transaction routed through a custom currency
	//   identity (CustomCurrencyIdentity is part of the persisted route,
	//   so it must also be part of the idempotency fingerprint)
	// when:
	// - the exact same custom-currency financial input is committed twice
	// then:
	// - the second call replays the original group without another
	//   booking, proving the fingerprint is stable across replays for
	...
	group := WithIdempotencyKey(
		"issue:custom-currency:sequential",
		GroupInputs(env.Namespace, nil, inputs...),
	)
	...
	require.NoError(t, err)
	require.Equal(t, float64(50), env.SumBalance(t, fboSubAccount).InexactFloat64())
}
			...
			retry: func(t *testing.T, env *transactionsTestEnv) []ledger.TransactionInput {
				return env.resolve(
					...
					IssueCustomerReceivableTemplate{
						...
						CustomCurrency: env.CustomCurrencyForRoute(currencyx.Code("ACME")),
					},
				)
			},
		},
