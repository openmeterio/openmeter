...
Use persisted ledger transactions and entries (the journal) as the source of
both amounts and unwind history. New credit collections carry an immutable
`origin_id` on their ledger entries. Backfill, recognition, and corrections
...
3. A purchase selects source-less receivable and accrued balances grouped by
   spend **and origin**, in original collection recording order. The earliest
   entry recording time in the original uncovered bucket, then origin ID,
   orders new origins; legacy roots retain their creation time and ID. Both
   participate in one FIFO queue. Remaining capacity is the ledger sum, bounded
   by matching receivable, accrued, and the purchase remainder. Partial backfill
   changes only the selected origin's source/cost-basis bucket, preserving its
   original place. Tax splitting stays inside the selected occurrence.
   Legacy selections retain #5092's exact segment IDs and amounts; persistence
   must reject stale selections and roll back the whole purchase.
   After accrued backing, attribute eligible receivable-only remainder before
   issuing excess FBO. This creates neither accrued backing nor lineage state.
...
origin is not a retry key. Several origins in one batch cannot independently
claim the same remaining entry. All reads, postings, breakage changes, and
billing realizations share a transaction; a failed invariant aborts the whole operation.
...
## Revalidation after #5083 and #5092

The initial experiment incorrectly accepted proportional 12.5/12.5 posting
against two 20-credit advances. That was an accounting bug, not a reason to
change the expected behavior. Purchase 25 must back the older advance for 20
and the newer for 5, in both ledger and legacy lineage. Interleaved A/B/A runs
remain distinct; correcting B cannot reverse A's backing or earnings.

The design still holds: immutable origin identifies the occurrence, original
recording time supplies FIFO order, and journal sums supply remaining amounts.
No replacement segment timestamp or charge-wide balance is needed for new
collections. Entries must remain distinct by origin even when posting routes
match. The implementation retains zero-rounded purchase validation, exact
source/spend/route corrections, receivable-only attribution, and nominal-credit
versus fiat settlement semantics from the merged fixes.

Legacy recognition still has the independently tracked [allocation mismatch
in #5091](https://github.com/openmeterio/openmeter/pull/5091): after recognizing
8 paid credits, lineage can mark 2 promotional plus 6 paid credits recognized.
Minting origins for new collections does not repair that history or legacy
writer. Fix that defect before production cutover. The new-origin regression
proves that 2 nil-cost promotional credits stay accrued while only the 8-credit
paid advance backfill is recognized and later unwound.
