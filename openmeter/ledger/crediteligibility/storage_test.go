...
func TestFilterStorage(t *testing.T) {
	for _, features := range [][]string{nil, {}, {"output", "input"}} {
		input := crediteligibility.Filters{Features: features}
...
func TestStoredFiltersVersion1(t *testing.T) {
	// Given a v1 row, decoding retains its format and normalizes its dimensions.
	var stored crediteligibility.StoredFilters
...
func TestStoredFiltersRejectUnsupportedVersions(t *testing.T) {
	...
	for _, version := range []crediteligibility.FiltersVersion{0, 99} {
		_, err := json.Marshal(crediteligibility.StoredFilters{Version: version})
		require.Error(t, err)
	}
}
