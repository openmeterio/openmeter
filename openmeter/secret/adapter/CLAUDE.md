...
**Compile-time interface assertion at package top** — Every adapter file declares var _ secret.Adapter = (*adapter)(nil) to catch interface drift at compile time. When secret.Adapter gains a new method the assertion fails here first. (`var _ secret.Adapter = (*adapter)(nil)`)
...
**SecretID IS the plaintext value** — CreateAppSecret/UpdateAppSecret return secretentity.NewSecretID(input.AppID, input.Value, input.Key) — the ID carries the raw secret. GetAppSecret reconstructs Secret.Value from input.ID. Intentional stub behaviour; a real adapter must never do this. (`func (a adapter) CreateAppSecret(ctx context.Context, input secretentity.CreateAppSecretInput) (secretentity.SecretID, error) { return secretentity.NewSecretID(input.AppID, input.Value, input.Key), nil }`)
...
| `secret.go` | All four CRUD methods: CreateAppSecret, UpdateAppSecret, GetAppSecret, DeleteAppSecret. All no-ops that encode/decode value in SecretID. | GetAppSecret sets Secret.Value = input.ID (plaintext stored as the ID). DeleteAppSecret always returns nil. Correct for the stub but wrong for a real backend. |
...
- Adding validation logic here — validation belongs in secret/service, not the adapter
- Constructing SecretID with a hashed or encrypted value instead of the raw value — the stub contract relies on ID == plaintext value

## Decisions

- **SecretID.ID encodes the plaintext value in the stub adapter** — Allows the full app to function without an external secret store during development; GetAppSecret reconstructs the value from the ID without state, keeping the adapter stateless.
