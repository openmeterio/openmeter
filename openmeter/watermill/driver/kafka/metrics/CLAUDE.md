...
**Mutex-guarded registration** — GetOrRegister and Register acquire r.mu before checking or writing to the embedded Registry, preventing races on concurrent Sarama metric registration. (`r.mu.Lock(); defer r.mu.Unlock() at the top of both GetOrRegister and Register`)
...
| `README.md` | Documents the known limitations: context is always context.Background() and metric registration errors are only logged, never propagated to callers. | If OTel metric registration errors are silently swallowed by errorHandler, callers see no panic but metrics are missing entirely. |
...
- Using a real propagated context instead of context.Background() in wrapped methods — go-metrics interfaces have no context parameter so callers cannot propagate one.

## Decisions

- **Embed metrics.Registry rather than re-implementing it** — All read/iteration/unregister operations on the registry are unchanged; only the write path (Register, GetOrRegister) needs interception to wrap new metrics, minimising the surface area of this adapter.
...
- **context.Background() in all OTel calls** — go-metrics mutation methods (Mark, Inc, Update) carry no context; propagating caller context is structurally impossible without changing the go-metrics interface, which would require forking Sarama.

## Example: Register a custom go-metrics registry that forwards to OTel, filtering noise metrics via NameTransformFn
