...
// FIXME: consolidate http mappers by moving common ones to productcatalog pkg to avoid duplicating
...
func AsCreateAddonRequest(a api.AddonCreate, namespace string) (CreateAddonRequest, error) {
	...
	req.RateCards, err = AsRateCards(a.RateCards)
	if err != nil {
		return req, err
	}
...
func AsUpdateAddonRequest(a api.AddonReplaceUpdate, namespace string, addonID string) (UpdateAddonRequest, error) {
	...
	if len(a.RateCards) > 0 {
		rateCards, err := AsRateCards(a.RateCards)
		if err != nil {
			return req, err
		}
