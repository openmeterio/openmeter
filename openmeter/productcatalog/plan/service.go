...
func (i UpdatePlanInput) applyTo(p productcatalog.Plan) productcatalog.Plan {
	...
	// labels is cleared, so validation must see the cleared state.
...
// UpdatePlanEffectivePeriodInput moves a Plan version's schedule without touching any of its
// content. Publishing and archiving only need to shift the effective period, so they use this
// instead of UpdatePlanInput: UpdatePlanInput carries the complete replacement state of a PUT
// request and clears every field the caller omitted, which would wipe the plan's name,
