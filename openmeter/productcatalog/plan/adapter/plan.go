...
func (a *adapter) ListPlans(ctx context.Context, params plan.ListPlansInput) (pagination.Result[plan.Plan], error) {
	fn := func(ctx context.Context, a *adapter) (pagination.Result[plan.Plan], error) {
		...
		if len(params.Currencies) > 0 {
			currencyFilter := filter.FilterString{In: &params.Currencies}
			if currencyPredicate := planCurrencyPredicate(&currencyFilter); currencyPredicate != nil {
				orFilters = append(orFilters, *currencyPredicate)
			}
		}
		...
		if currencyPredicate := planCurrencyPredicate(params.Currency); currencyPredicate != nil {
...
func (a *adapter) CreatePlan(ctx context.Context, params plan.CreatePlanInput) (*plan.Plan, error) {
	fn := func(ctx context.Context, a *adapter) (*plan.Plan, error) {
		...
		if err := a.validatePlanReferences(ctx, params.Namespace, params.Plan); err != nil {
			return nil, fmt.Errorf("invalid create Plan references: %w", err)
		}
...
func (a *adapter) UpdatePlan(ctx context.Context, params plan.UpdatePlanInput) (*plan.Plan, error) {
	fn := func(ctx context.Context, a *adapter) (*plan.Plan, error) {
		...
		if params.Phases != nil {
			if err := a.validatePlanReferences(ctx, params.Namespace, productcatalog.Plan{Phases: *params.Phases}); err != nil {
				return nil, fmt.Errorf("invalid update Plan references: %w", err)
			}
		}
		...
		_, err = a.db.PlanPhase.Delete().
			Where(phasedb.Namespace(params.Namespace), phasedb.PlanID(p.ID)).
			Exec(ctx)
		if err != nil {
