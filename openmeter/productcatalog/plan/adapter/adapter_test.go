...
func TestPlanWriteRejectsForeignReferences(t *testing.T) {
	// given:
	// - custom currencies, a feature, and a tax code owned by another namespace
	// when:
	// - direct repository create or update inputs reference those entities
	// then:
	// - the mutation is rejected and no plan graph changes are persisted
	...
	foreignCurrency, err := env.Currency.CreateCurrency(t.Context(), currencytestutils.NewCreateCurrencyInput(
		foreignNamespace,
		"FOREIGN_CREDITS",
		"Foreign credits",
		"fcr",
	...
	foreignFeature, err := env.Feature.CreateFeature(t.Context(), pctestutils.NewTestFeature(t, foreignNamespace))
	require.NoError(t, err)

	foreignTaxCode, err := env.TaxCode.CreateTaxCode(t.Context(), taxcode.CreateTaxCodeInput{
		Namespace: foreignNamespace,
		Key:       "foreign-plan-tax-code",
		Name:      "Foreign plan tax code",
	})
	require.NoError(t, err)
	...
	type testCase struct {
		...
		update      bool
		expectedErr error
		mutate      func(*plan.CreatePlanInput)
	}

	testCases := []testCase{
		{
			name:        "plan custom currency",
			key:         "foreign-plan-currency",
			expectedErr: productcatalog.ErrCurrencyNotFound,
			mutate: func(input *plan.CreatePlanInput) {
				input.Currency = foreignCurrency.Reference()
			},
		},
		{
			name:        "rate card custom currency",
			key:         "foreign-rate-card-currency",
			update:      true,
			expectedErr: productcatalog.ErrCurrencyNotFound,
			mutate: func(input *plan.CreatePlanInput) {
				reference := foreignCurrency.Reference()
				rateCard := input.Phases[0].RateCards[0].(*productcatalog.FlatFeeRateCard)
		...
		},
		{
			...
			mutate: func(input *plan.CreatePlanInput) {
				rateCard := input.Phases[0].RateCards[0].(*productcatalog.FlatFeeRateCard)
				rateCard.RateCardMeta.Key = foreignFeature.Key
				rateCard.FeatureKey = lo.ToPtr(foreignFeature.Key)
				rateCard.FeatureID = lo.ToPtr(foreignFeature.ID)
				rateCard.EntitlementTemplate = productcatalog.NewEntitlementTemplateFrom(productcatalog.BooleanEntitlementTemplate{})
			},
		},
		{
			name:        "tax code",
			key:         "foreign-tax-code",
			update:      true,
			expectedErr: taxcode.ErrTaxCodeNotFound,
			mutate: func(input *plan.CreatePlanInput) {
				rateCard := input.Phases[0].RateCards[0].(*productcatalog.FlatFeeRateCard)
				rateCard.TaxConfig = &productcatalog.TaxConfig{
					TaxCodeID: lo.ToPtr(foreignTaxCode.ID),
				}
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run("create/"+tc.name, func(t *testing.T) {
			input := pctestutils.NewTestPlan(t, namespace, pctestutils.WithPlanKey(tc.key))
			tc.mutate(&input)

			_, err := env.PlanRepository.CreatePlan(t.Context(), input)
			require.Error(t, err)
	...
	base, err := env.PlanRepository.CreatePlan(t.Context(), pctestutils.NewTestPlan(
		t,
		namespace,
		pctestutils.WithPlanKey("foreign-reference-update-base"),
	))
	require.NoError(t, err)
	...
	for _, tc := range testCases {
		...
		t.Run("update/"+tc.name, func(t *testing.T) {
			input := pctestutils.NewTestPlan(t, namespace, pctestutils.WithPlanKey(tc.key+"-update"))
			tc.mutate(&input)

			_, err := env.PlanRepository.UpdatePlan(t.Context(), plan.UpdatePlanInput{
				NamespacedID: base.NamespacedID,
				Phases:       &input.Phases,
			})
			require.Error(t, err)
			require.True(t, models.IsGenericValidationError(err))
			...
			require.NoError(t, err)
			plan.AssertPlanEqual(t, *base, *fetched)
		})
	}
}

func TestPlanAddonWriteRejectsForeignParents(t *testing.T) {
	// given:
	// - plans and add-ons in two different namespaces
	// when:
	// - a direct repository create mixes a local assignment with a foreign parent
	// then:
	// - the foreign parent is treated as missing and no assignment is written
	env := pctestutils.NewTestEnv(t)
	t.Cleanup(func() { env.Close(t) })
	...
	localPlan, err := env.PlanRepository.CreatePlan(t.Context(), pctestutils.NewTestPlan(
		t,
		namespace,
		pctestutils.WithPlanKey("local-assignment-plan"),
	))
	require.NoError(t, err)
	foreignPlan, err := env.PlanRepository.CreatePlan(t.Context(), pctestutils.NewTestPlan(
		t,
		foreignNamespace,
		pctestutils.WithPlanKey("foreign-assignment-plan"),
	))
	require.NoError(t, err)

	newAddonInput := func(namespace string) addon.CreateAddonInput {
		return pctestutils.NewTestAddon(t, namespace, &productcatalog.FlatFeeRateCard{
			RateCardMeta: productcatalog.RateCardMeta{
				Key:  "assignment-rate-card",
				Name: "Assignment rate card",
	...
	localAddon, err := env.AddonRepository.CreateAddon(t.Context(), newAddonInput(namespace))
	require.NoError(t, err)
	foreignAddon, err := env.AddonRepository.CreateAddon(t.Context(), newAddonInput(foreignNamespace))
	require.NoError(t, err)
	...
	testCases := []struct {
	...
	}{
		{
			...
			fromPlanPhase: foreignPlan.Phases[0].Key,
			isExpected:    plan.IsNotFound,
		},
		{
			name:          "foreign addon",
			planID:        localPlan.ID,
			addonID:       foreignAddon.ID,
			fromPlanPhase: localPlan.Phases[0].Key,
			isExpected:    addon.IsNotFound,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := env.PlanAddonRepository.CreatePlanAddon(t.Context(), planaddon.CreatePlanAddonInput{
				NamespacedModel: models.NamespacedModel{Namespace: namespace},
				PlanID:          tc.planID,
				AddonID:         tc.addonID,
				FromPlanPhase:   tc.fromPlanPhase,
			})
			require.Error(t, err)
			require.True(t, models.IsGenericNotFoundError(err))
			require.True(t, tc.isExpected(err))

			count, err := env.Client.PlanAddon.Query().Count(t.Context())
			require.NoError(t, err)
