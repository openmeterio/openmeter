// Package unitconfig holds the UnitConfig domain DTO — the per-rate-card rule that
// converts a raw metered quantity into a billing quantity before pricing and
// entitlement evaluation.
//
// It lives in its own leaf package (imports only pkg/models and decimal/lo) so that
// packages BELOW productcatalog in the import graph — entitlement, credit/grant,
// credit/balance — can use the typed value without an import cycle. The
// productcatalog package re-exports these names as aliases, so existing
// productcatalog.UnitConfig references keep working unchanged.
package unitconfig
