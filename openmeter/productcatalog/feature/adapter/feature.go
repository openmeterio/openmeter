...
func (a *adapter) CreateFeature(ctx context.Context, feat feature.CreateFeatureInputs) (feature.Feature, error) {
	query := a.db.Feature.Create().
		SetName(feat.Name).
		SetKey(feat.Key).
...
func (a *adapter) GetByIdOrKey(ctx context.Context, namespace string, idOrKey string, includeArchived bool) (*feature.Feature, error) {
	query := a.db.Feature.Query().
...
func (a *adapter) ArchiveFeature(ctx context.Context, params feature.ArchiveFeatureInput) error {
	f, err := a.GetByIdOrKey(ctx, params.Namespace, params.ID, true)
	if err != nil {
		return err
	}
	...
	planReferencesIt, err := a.db.Plan.Query().
	...
	subsReferencesIt, err := a.db.Subscription.Query().
...
func (a *adapter) HasActiveFeatureForMeter(ctx context.Context, namespace string, meterSlug string) (bool, error) {
...
func (a *adapter) ListFeatures(ctx context.Context, params feature.ListFeaturesParams) (pagination.Result[feature.Feature], error) {
	query := a.db.Feature.Query().
		Where(db_feature.Namespace(params.Namespace))
