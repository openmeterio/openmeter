...
// Paginate runs the query and returns a paginated response.
// If page is its 0 value then it will return all the items and populate the response page accordingly.
func (bicnl *BillingInvoiceCreditNoteLineQuery) Paginate(ctx context.Context, page pagination.Page) (pagination.PagedResponse[*BillingInvoiceCreditNoteLine], error) {
	// Get the limit and offset
	limit, offset := page.Limit(), page.Offset()

	// Unset previous pagination settings
	zero := 0
	bicnl.ctx.Offset = &zero
	bicnl.ctx.Limit = &zero

	// Create duplicate of the query to run for
	countQuery := bicnl.Clone()
	pagedQuery := bicnl

	// Unset ordering for count query
	countQuery.order = nil

	pagedResponse := pagination.PagedResponse[*BillingInvoiceCreditNoteLine]{
		Page: page,
	}
...
// type check
var _ pagination.Paginator[*BillingInvoiceCreditNoteLine] = (*BillingInvoiceCreditNoteLineQuery)(nil)
