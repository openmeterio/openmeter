...
// Cursor runs the query and returns a cursor-paginated response.
// Ordering is always by created_at asc, id asc.
func (_m *ChargeUsageBasedDetailedLineQuery) Cursor(ctx context.Context, cursor *pagination.Cursor) (pagination.Result[*ChargeUsageBasedDetailedLine], error) {
	if cursor != nil {
		if err := cursor.Validate(); err != nil {
			return pagination.Result[*ChargeUsageBasedDetailedLine]{}, fmt.Errorf("invalid cursor: %w", err)
		}
	...
	items, err := _m.All(ctx)
	if err != nil {
		return pagination.Result[*ChargeUsageBasedDetailedLine]{}, err
	}

	if items == nil {
		items = make([]*ChargeUsageBasedDetailedLine, 0)
	}

	result := pagination.Result[*ChargeUsageBasedDetailedLine]{
		Items: items,
	}
