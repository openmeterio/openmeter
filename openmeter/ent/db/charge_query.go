...
func (_q *ChargeQuery) QueryBillingGatheringInvoiceLines() *BillingGatheringInvoiceLineQuery {
...
func (_q *ChargeQuery) WithBillingGatheringInvoiceLines(opts ...func(*BillingGatheringInvoiceLineQuery)) *ChargeQuery {
	query := (&BillingGatheringInvoiceLineClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChargeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Charge, error) {
	...
	if query := _q.withBillingGatheringInvoiceLines; query != nil {
		if err := _q.loadBillingGatheringInvoiceLines(ctx, query, nodes,
			func(n *Charge) { n.Edges.BillingGatheringInvoiceLines = []*BillingGatheringInvoiceLine{} },
			func(n *Charge, e *BillingGatheringInvoiceLine) {
...
func (_q *ChargeQuery) loadFlatFee(ctx context.Context, query *ChargeFlatFeeQuery, nodes []*Charge, init func(*Charge), assign func(*Charge, *ChargeFlatFee)) error {
	...
	for i := range nodes {
		if nodes[i].ChargeFlatFeeID == nil {
			continue
		}
		fk := *nodes[i].ChargeFlatFeeID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "charge_flat_fee_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChargeQuery) loadCreditPurchase(ctx context.Context, query *ChargeCreditPurchaseQuery, nodes []*Charge, init func(*Charge), assign func(*Charge, *ChargeCreditPurchase)) error {
	...
	nodeids := make(map[string][]*Charge)
	for i := range nodes {
		if nodes[i].ChargeCreditPurchaseID == nil {
			continue
		}
		fk := *nodes[i].ChargeCreditPurchaseID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "charge_credit_purchase_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChargeQuery) loadUsageBased(ctx context.Context, query *ChargeUsageBasedQuery, nodes []*Charge, init func(*Charge), assign func(*Charge, *ChargeUsageBased)) error {
	...
	nodeids := make(map[string][]*Charge)
	for i := range nodes {
		if nodes[i].ChargeUsageBasedID == nil {
			continue
		}
		fk := *nodes[i].ChargeUsageBasedID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "charge_usage_based_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChargeQuery) loadBillingGatheringInvoiceLines(ctx context.Context, query *BillingGatheringInvoiceLineQuery, nodes []*Charge, init func(*Charge), assign func(*Charge, *BillingGatheringInvoiceLine)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Charge)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(billinggatheringinvoiceline.FieldChargeID)
	}
	query.Where(predicate.BillingGatheringInvoiceLine(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(charge.BillingGatheringInvoiceLinesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ChargeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withFlatFee != nil {
			_spec.Node.AddColumnOnce(charge.FieldChargeFlatFeeID)
		}
		if _q.withCreditPurchase != nil {
			_spec.Node.AddColumnOnce(charge.FieldChargeCreditPurchaseID)
		}
		if _q.withUsageBased != nil {
			_spec.Node.AddColumnOnce(charge.FieldChargeUsageBasedID)
		}
	}
