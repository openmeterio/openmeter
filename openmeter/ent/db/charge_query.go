...
func (_q *ChargeQuery) QueryCreditPurchase() *ChargeCreditPurchaseQuery {
...
func (_q *ChargeQuery) WithCreditPurchase(opts ...func(*ChargeCreditPurchaseQuery)) *ChargeQuery {
	query := (&ChargeCreditPurchaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ChargeQuery) loadCreditPurchase(ctx context.Context, query *ChargeCreditPurchaseQuery, nodes []*Charge, init func(*Charge), assign func(*Charge, *ChargeCreditPurchase)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Charge)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
