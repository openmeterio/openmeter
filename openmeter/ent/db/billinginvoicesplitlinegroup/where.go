...
// HasUsageBasedLineConfig applies the HasEdge predicate on the "usage_based_line_config" edge.
func HasUsageBasedLineConfig() predicate.BillingInvoiceSplitLineGroup {
	return predicate.BillingInvoiceSplitLineGroup(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, UsageBasedLineConfigTable, UsageBasedLineConfigColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasUsageBasedLineConfigWith applies the HasEdge predicate on the "usage_based_line_config" edge with a given conditions (other predicates).
func HasUsageBasedLineConfigWith(preds ...predicate.BillingInvoiceUsageBasedLineConfig) predicate.BillingInvoiceSplitLineGroup {
	return predicate.BillingInvoiceSplitLineGroup(func(s *sql.Selector) {
		step := newUsageBasedLineConfigStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
