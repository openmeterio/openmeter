...
func (_c *BillingInvoiceSplitLineGroupCreate) AddUsageBasedLineConfigIDs(ids ...string) *BillingInvoiceSplitLineGroupCreate {
	_c.mutation.AddUsageBasedLineConfigIDs(ids...)
	return _c
}

// AddUsageBasedLineConfig adds the "usage_based_line_config" edges to the BillingInvoiceUsageBasedLineConfig entity.
func (_c *BillingInvoiceSplitLineGroupCreate) AddUsageBasedLineConfig(v ...*BillingInvoiceUsageBasedLineConfig) *BillingInvoiceSplitLineGroupCreate {
	ids := make([]string, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddUsageBasedLineConfigIDs(ids...)
}
...
func (_c *BillingInvoiceSplitLineGroupCreate) createSpec() (*BillingInvoiceSplitLineGroup, *sqlgraph.CreateSpec, error) {
	...
	if nodes := _c.mutation.UsageBasedLineConfigIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
