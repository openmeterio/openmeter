...
func (*ChargesSearchV1) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case chargessearchv1.FieldSubscriptionPlan:
			values[i] = chargessearchv1.ValueScanner.SubscriptionPlan.ScanValue()
		default:
...
func (_m *ChargesSearchV1) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case chargessearchv1.FieldBaseIntentDeletedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field base_intent_deleted_at", values[i])
			} else if value.Valid {
		...
		case chargessearchv1.FieldSubscriptionPlan:
			if value, err := chargessearchv1.ValueScanner.SubscriptionPlan.FromValue(values[i]); err != nil {
				return err
			} else {
