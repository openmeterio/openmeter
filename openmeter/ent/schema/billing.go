...
func (BillingInvoiceUsageBasedLineConfig) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("split_line_group", BillingInvoiceSplitLineGroup.Type).
			Ref("usage_based_line_config").
...
func (BillingInvoiceSplitLineGroup) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("customer", Customer.Type).
			...
			Unique().
			Required(),
		edge.To("usage_based_line_config", BillingInvoiceUsageBasedLineConfig.Type),
	}
}
