...
**NoopRequestValidator embedding** — Embed customer.NoopRequestValidator so only overridden methods need implementation; unimplemented lifecycle methods default to no-op. (`type Validator struct { customer.NoopRequestValidator; subscriptionService subscription.Service; customerService customer.Service }`)
**Interface assertion at package level** — Declare var _ customer.RequestValidator = (*Validator)(nil) at package top for compile-time interface satisfaction. (`var _ customer.RequestValidator = (*Validator)(nil)`)
...
| File | Role | Watch For |
|------|------|-----------|
| `validator.go` | Single file implementing customer.RequestValidator with ValidateUpdateCustomer and ValidateDeleteCustomer logic. | ValidateUpdateCustomer fetches the current customer only when UsageAttribution.SubjectKeys is non-nil — skipping this branch for other mutations is intentional. Errors must use models.NewGenericNotFoundError / models.NewGenericPreConditionFailedError for correct HTTP status mapping. |

## Anti-Patterns

- Returning plain fmt.Errorf for not-found or pre-condition errors — must use models.NewGenericNotFoundError / models.NewGenericPreConditionFailedError so HTTP mapping works
...
- Checking active subscriptions without the ActiveAt filter — returns already-cancelled subscriptions
...
- **Validator is registered via customer.Service.RegisterRequestValidator() in app/common wiring, not hardcoded in the customer package.** — Avoids a circular import between subscription and customer packages; subscription registers the constraint at startup through the registry pattern.
