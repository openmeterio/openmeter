...
**NoOpSubscriptionCommandHook embedding** — Embed subscription.NoOpSubscriptionCommandHook so only overridden Before*/After* methods need implementation. (`type SubscriptionUniqueConstraintValidator struct { subscription.NoOpSubscriptionCommandHook; Config SubscriptionUniqueConstraintValidatorConfig }`)
...
**Feature-flag-gated validation path** — Check ffx.Service.IsFeatureEnabled for subscription.MultiSubscriptionEnabledFF before choosing a validation path; BeforeUpdate skips all validation when multi-subscription is disabled. (`multiSubscriptionEnabled, err := v.Config.FeatureFlags.IsFeatureEnabled(ctx, subscription.MultiSubscriptionEnabledFF)`)
...
**pipelineAfter reuse across post-mutation hooks** — AfterCreate, AfterUpdate, AfterCancel, AfterContinue, and BeforeDelete all delegate to the single pipelineAfter method; includeSubViewUnique (lo.UniqBy) prevents double-counting. (`func (v SubscriptionUniqueConstraintValidator) AfterCreate(ctx context.Context, view subscription.SubscriptionView) error { return v.pipelineAfter(ctx, view) }`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `validator.go` | Implements all Before*/After* hook methods; each builds a collect->map->validate->include pipeline. | pipelineAfter is reused for AfterCreate, AfterUpdate, AfterCancel, AfterContinue, and BeforeDelete — any change to post-mutation validation affects all five hooks simultaneously. |
| `components.go` | Low-level pipeline building blocks (collect, map, filter, include) used by validator.go; no business rules here. | includeSubViewUnique uses lo.UniqBy by subscription ID to prevent double-counting; removing that dedup causes false-positive uniqueness errors. |
...
- Adding business logic directly into components.go — keep domain decisions in validator.go

## Decisions

- **Components are separated into components.go to keep validator.go focused on validation pipeline logic.** — Improves readability and testability of each pipeline step in isolation.
...
- **Hook is registered via subscription.Service.RegisterHook() at wiring time in app/common, not inside the subscription package.** — Keeps the uniqueness constraint out of the core subscription package to avoid tight coupling and import cycles.
