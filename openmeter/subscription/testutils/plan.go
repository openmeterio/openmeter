			...
			RateCards: []*RateCard{
				{
					...
					SubscriptionItemCreateInput: subscription.CreateSubscriptionItemPlanInput{
						...
						FeatureKey: lo.ToPtr("test-feature-1"),
					},
				},
			},
		},
...
func (a *planAdapter) AddPlan(plan *Plan) {
	...
	if _, ok := a.store[plan.Ref.Key]; !ok {
		a.store[plan.Ref.Key] = make(map[int]*Plan)
	}

	a.store[plan.Ref.Key][plan.Ref.Version] = plan
}
