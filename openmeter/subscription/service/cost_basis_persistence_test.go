	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			planInput := subscriptiontestutils.BuildTestPlanInput(t).
				...
				Build()
			planInput.Plan.Currency = customCurrency.Reference()
...
func TestSubscriptionUpdatePinsNewCostBasisPairAtEffectiveTime(t *testing.T) {
	...
	customer := deps.CustomerAdapter.CreateExampleCustomer(t)
	creditsCurrency, err := deps.CurrencyService.CreateCurrency(t.Context(), currenciestestutils.NewCreateCurrencyInput(
		subscriptiontestutils.ExampleNamespace, "CREDITS", "Credits", "CR",
	))
	require.NoError(t, err)
	pointsCurrency, err := deps.CurrencyService.CreateCurrency(t.Context(), currenciestestutils.NewCreateCurrencyInput(
		subscriptiontestutils.ExampleNamespace, "POINTS", "Points", "PT",
	))
	require.NoError(t, err)
	...
	planInput := subscriptiontestutils.BuildTestPlanInput(t).
		...
		Build()
	planInput.Plan.Currency = creditsCurrency.Reference()
	plan := deps.PlanHelper.CreatePlan(t, planInput)
	...
	phase.ItemsByKey["points-fee"] = []*subscription.SubscriptionItemSpec{
		{
			CreateSubscriptionItemInput: subscription.CreateSubscriptionItemInput{
				CreateSubscriptionItemPlanInput: subscription.CreateSubscriptionItemPlanInput{
					...
					ItemKey:  "points-fee",
					RateCard: newCostBasisTestRateCard(t, "points-fee", lo.ToPtr(pointsCurrency.Reference())),
				},
				Annotations: models.Annotations{},
			},
		},
...
func newCostBasisTestRateCard(t *testing.T, key string, currencyReference *currencies.CurrencyReference) productcatalog.RateCard {
	t.Helper()
