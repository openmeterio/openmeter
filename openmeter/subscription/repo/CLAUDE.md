...
> Ent/PostgreSQL persistence layer for subscriptions, phases, and items. Implements subscription.SubscriptionRepository, SubscriptionPhaseRepository, and SubscriptionItemRepository with shared TransactingRepo + Tx/WithTx/Self triad.
...
**Soft-delete via SetDeletedAt** — Deletes set DeletedAt rather than removing rows. Queries filter with DeletedAtIsNil() or DeletedAtGT(now) to exclude soft-deleted records. (`err := repo.db.SubscriptionItem.UpdateOneID(input.ID).SetDeletedAt(at).Exec(ctx)`)
**DB error mapping to domain errors** — db.IsNotFound(err) is checked after every query and mapped to domain-specific not-found errors. (`if db.IsNotFound(err) { return subscription.SubscriptionItem{}, subscription.NewItemNotFoundError(id.ID) }`)
...
| `mapping.go` | Converts Ent DB rows to domain types; MapDBSubscriptionItem requires eager-loaded Phase edge (PhaseOrErr). | Always call WithPhase() on item queries; missing edge causes MapDBSubscriptionItem to error at runtime. |
