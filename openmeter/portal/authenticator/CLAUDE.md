...
**Security scheme name matched by prefix split** — getAuthenticatorFunc splits api.PortalTokenAuthScopes on '.' and matches by first segment. Adding a new auth scheme requires a new case in this switch. (`case strings.Split(string(api.PortalTokenAuthScopes), ".")[0]: return a.verifyPortalToken`)
