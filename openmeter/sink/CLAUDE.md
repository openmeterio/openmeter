...
> High-throughput Kafka→ClickHouse sink worker: consumes raw CloudEvent messages from Kafka partitions, deduplicates via Redis or in-memory, batch-inserts into ClickHouse via streaming.Connector, then fires post-flush callbacks (FlushEventHandler) so the balance-worker can recalculate entitlements. Primary constraint: exactly-once guarantee requires strict flush ordering (ClickHouse → Kafka offset commit → Redis dedupe).

## Patterns

**SinkConfig.Validate() before any Sink construction** — All Sink configuration is validated in SinkConfig.Validate() before NewSink returns. Every required field has an explicit nil/zero check; callers must not pass partially-configured SinkConfig. (`func NewSink(config SinkConfig) (*Sink, error) { if err := config.Validate(); err != nil { return nil, fmt.Errorf(...) } }`)
**Three-phase flush ordering: storage → offset commit → Redis dedupe** — flush() always writes ClickHouse first, then stores Kafka offsets, then sets Redis dedupe keys. Reversing this order breaks the exactly-once guarantee. (`// 1. persistToStorage → 2. Consumer.StoreMessage → 3. dedupeSet`)
**FlushEventHandler called in a goroutine after flush, never blocking** — OnFlushSuccess is dispatched in a separate goroutine with a FlushSuccessTimeout context. Never call FlushEventHandler synchronously from flush(). (`go func() { ctx, cancel := context.WithTimeout(ctx, s.config.FlushSuccessTimeout); defer cancel(); s.config.FlushEventHandler.OnFlushSuccess(ctx, messages) }()`)
**NamespacedMeterCache for meter definitions, refreshed periodically** — Meter definitions are cached in NamespacedMeterCache and periodically re-fetched at MeterRefetchInterval. Access meter definitions via meterCache.GetMetersByType(), never query meter.Service directly in the hot path. (`meters, err := s.meterCache.GetMetersByType(message)`)
**Storage interface for ClickHouse — inject via SinkConfig.Storage** — ClickHouseStorage implements Storage. Tests can inject a mock. Never instantiate ClickHouseStorage directly inside sink.go. (`type Storage interface { BatchInsert(ctx context.Context, messages []sinkmodels.SinkMessage) error }`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `sink.go` | Main Sink struct, SinkConfig, NewSink constructor, flush() orchestration, Run() event loop. The flush mutex (s.mu) serialises partition pause/resume with close. | Any change to flush ordering risks exactly-once violation; the mu.Lock covers the full ClickHouse+offset+Redis sequence. |
...
| `flushhandler/handler.go` | FlushEventHandler interface + buffered async impl. Two-phase shutdown: stopChan signal then drain loop. | WaitForDrain() must be called before process exit or buffered messages will be dropped. |
...
func (h *myHandler) OnFlushSuccess(ctx context.Context, msgs []sinkmodels.SinkMessage) error {
    // process msgs; ctx has FlushSuccessTimeout deadline
    return nil
}

// Wire via flushhandler.FlushEventHandlers multiplexer:
