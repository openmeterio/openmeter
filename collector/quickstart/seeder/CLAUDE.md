...
**Environment-variable-driven configuration** — Every runtime-variable value uses Benthos interpolation syntax `${VAR:default}`. Adding new tunables must follow this pattern — no hardcoded values. (`count: ${SEEDER_COUNT:0}  interval: "${SEEDER_INTERVAL:50ms}"`)
