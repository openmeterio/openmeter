...
- **Sequential Atlas migration filenames + atlas.sum chain hashing guarantee merge collisions on long-lived feature branches.**
  ...
  - *Fix direction:*
    ...
    - For long-lived branches with schema changes, rebase daily to minimise the cost of regeneration.
- **Tests or build invocations that omit -tags=dynamic fail to link confluent-kafka-go against librdkafka, producing confusing link errors that look unrelated to Kafka.**
  - *Evidence:* Makefile sets GO_BUILD_FLAGS = -tags=dynamic for every build and test target.; AGENTS.md: 'Always build with -tags=dynamic to link confluent-kafka-go against the local librdkafka.'; Codex / agent shells often miss .envrc loading, leading to ad-hoc go test invocations that drop the tag.
  - *Root cause:* confluent-kafka-go's dynamic linking against librdkafka is a CGO requirement enforced only at link time, not by Go modules. Each new test target or shell-based invocation must inherit the tag explicitly.
  - *Fix direction:*
    - Always run tests through `make test` / `make test-nocache` which set -tags=dynamic and POSTGRES_HOST.
    - If running ad-hoc, use `nix develop --impure .#ci -c go test -tags=dynamic ...`.
    - Document the tag in any per-package CLAUDE.md that covers Kafka-touching code.
    - Consider a small wrapper script (tools/run-test.sh) that injects the tag if missing.
- **Cross-domain hook/validator registration is implemented as side-effects inside Wire provider functions; omitting the provider in a binary's wire.Build silently drops the hook with no compile error.**
  ...
  - *Fix direction:*
    ...
    - Add an integration test per binary that asserts customerService.HookCount() matches the expected count for that binary's role.
...
- **EventName prefix-based topic routing in eventbus.GeneratePublishTopic falls through to SystemEventsTopic by default; an event family that forgets to declare a recognised EventVersionSubsystem silently misroutes instead of failing fast.**
  ...
  - *Root cause:* Default-case fallback to SystemEventsTopic was a pragmatic choice for genuine system events but means no error fires for misnamed event families; the prefix match is structurally fragile.
  - *Fix direction:*
