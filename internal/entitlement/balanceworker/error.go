...
func (w *Worker) newEntitlementProcessingFailedError(entitlements []IngestEventDataResponse, ogErr error) error {
	...
	iEvents := slicesx.Map(
		...
		func(ent IngestEventDataResponse) ingestnotification.IngestEventData {
			// We can optimise this if needed so we group the events more efficiently
	...
	event, err := spec.NewCloudEvent(spec.EventSpec{
		...
		Source: spec.ComposeResourcePathRaw(string(ingestnotification.BatchedIngestEvent{}.Spec().Subsystem)),
	}, ingestnotification.BatchedIngestEvent{
		Events: iEvents,
	})
