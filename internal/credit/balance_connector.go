...
type BalanceConnector interface {
	...
	ResetUsageForOwner(ctx context.Context, owner NamespacedGrantOwner, params UsageResetParams) (balanceAfterReset *GrantBalanceSnapshot, err error)
}
...
func (m *balanceConnector) ResetUsageForOwner(ctx context.Context, owner NamespacedGrantOwner, req UsageResetParams) (*GrantBalanceSnapshot, error) {
	// Cannot reset for the future
	if req.At.After(time.Now()) {
		return nil, &models.GenericUserError{Message: fmt.Sprintf("cannot reset at %s in the future", req.At)}
	}

	// TODO: enforce granularity (truncate)
	at := req.At.Truncate(time.Minute)
	...
	_, err = entutils.StartAndRunTx(ctx, m.balanceSnapshotConnector, func(txCtx context.Context, tx *entutils.TxDriver) (*GrantBalanceSnapshot, error) {
		...
		err = m.ownerConnector.EndCurrentUsagePeriodTx(ctx, tx, owner, EndCurrentUsagePeriodParams{
			At:                      at,
			RetainUsagePeriodAnchor: req.RetainUsagePeriodAnchor,
		})
		if err != nil {
			return nil, err
		}
