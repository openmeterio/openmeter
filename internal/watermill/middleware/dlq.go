...
var defaultProcessor DLQMsgProcessorFunc = func(original *message.Message, err error) []*message.Message {
	if retryable, ok := errorsx.ErrorAs[watermill.RetryableError](err); ok {
		...
		defer func() {
			...
			for _, m := range msgs {
				if publishErr := dlq.publishWithErr(m, err); publishErr != nil {
					dlqSuccess = false
					publishErr = errors.Wrap(publishErr, "cannot publish message to DLQ")
