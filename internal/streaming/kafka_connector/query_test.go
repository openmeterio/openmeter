...
func TestCloudEventsStreamQuery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: "CREATE STREAM IF NOT EXISTS OM_EVENTS ( SUBJECT STRING KEY, ID STRING, TYPE STRING, SOURCE STRING, TIME STRING, DATA STRING ) WITH ( KAFKA_TOPIC = 'om_events', VALUE_FORMAT = 'JSON_SR', PARTITIONS = 1 );",
		},
		{
			data: cloudEventsStreamQueryData{
				Topic:      "foo",
			...
			want: "CREATE STREAM IF NOT EXISTS OM_EVENTS ( SUBJECT STRING KEY, ID STRING, TYPE STRING, SOURCE STRING, TIME STRING, DATA STRING ) WITH ( KAFKA_TOPIC = 'foo', VALUE_FORMAT = 'JSON_SR', PARTITIONS = 2 );",
		},
	}
