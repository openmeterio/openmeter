...
BEGIN
  IF EXISTS (
    SELECT 1 FROM "charge_credit_purchases" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "charge_flat_fee_run_detailed_lines" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "charge_flat_fees" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "charge_usage_based" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "charge_usage_based_run_detailed_line" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "credit_realization_lineages" WHERE char_length("currency") > 3
    UNION ALL
    SELECT 1 FROM "ledger_breakage_records" WHERE char_length("currency") > 3
  ) THEN
    RAISE EXCEPTION 'cannot rollback custom currency ledger support: currency values longer than 3 characters exist; drain custom-currency data before retrying';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM "ledger_sub_account_routes"
    WHERE "source" IS NOT NULL OR "routing_key_version" = 'v3'
  ) THEN
    RAISE EXCEPTION 'cannot rollback custom currency ledger support: source-bearing or V3 ledger routes exist; downgrade routes before retrying';
  END IF;
END
$$;

-- drop dependent search view before narrowing charge currency columns
DROP VIEW IF EXISTS "charges_search_v1s";
-- reverse: modify "ledger_sub_account_routes" table
ALTER TABLE "ledger_sub_account_routes" DROP COLUMN "source";
...
-- recreate charges_search_v1s view after narrowing charge currency columns
