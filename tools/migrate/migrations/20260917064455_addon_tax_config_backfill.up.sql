...
-- JSON tax config on add-on rate cards. A NULL tax_config is valid and is left
-- untouched.
--
...
BEGIN
  ...
  IF tax_code_conflicts > 0 OR behavior_conflicts > 0 THEN
    RAISE EXCEPTION 'add-on tax config backfill: % row(s) have conflicting tax_code_id representations and % row(s) have conflicting tax behavior representations; repair them before re-running', tax_code_conflicts, behavior_conflicts;
  END IF;
END $$;
...
BEGIN
  ...
  IF unresolved_count > 0 THEN
    RAISE EXCEPTION 'add-on tax config backfill: % row(s) contain an embedded tax_code_id that cannot be resolved to a live tax code in the same namespace', unresolved_count;
  END IF;
END $$;
...
CREATE TEMP TABLE _addon_tax_config_backfill_pairs AS
SELECT DISTINCT
    r.namespace,
...
CREATE INDEX ON _addon_tax_config_backfill_pairs (namespace, stripe_code);
...
BEGIN
  ...
  FROM _addon_tax_config_backfill_pairs
  WHERE stripe_code !~ '^txcd_[0-9]{8}$';

  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'add-on tax config backfill: % (namespace, code) pair(s) carry a non-Stripe tax code; correct or clear these tax configs and re-run. Offending codes: %', invalid_count, invalid_codes;
  END IF;
END $$;

-- Resolve each pair to the entity selected by the read path's deterministic
-- tie-break.
DROP TABLE IF EXISTS _addon_tax_config_backfill_map;
CREATE TEMP TABLE _addon_tax_config_backfill_map (
...
INSERT INTO _addon_tax_config_backfill_map (namespace, stripe_code, winner_id)
WITH expanded AS (
    SELECT
        t.id,
        t.namespace,
...
ranked AS (
    ...
    FROM expanded e
    JOIN _addon_tax_config_backfill_pairs p
...
BEGIN
  ...
  FROM _addon_tax_config_backfill_pairs p
  JOIN tax_codes t
    ON t.namespace = p.namespace
   AND t.key = 'stripe_' || p.stripe_code
   AND t.deleted_at IS NULL
  LEFT JOIN _addon_tax_config_backfill_map m
    ON m.namespace = p.namespace
   AND m.stripe_code = p.stripe_code
  WHERE m.winner_id IS NULL;

  IF orphaned_count > 0 THEN
    RAISE EXCEPTION 'add-on tax config backfill: % orphaned auto-key(s); a live tax code entity holds the stripe_ key without the matching app mapping: %', orphaned_count, orphaned;
  END IF;
END $$;
...
WITH inserted AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM _addon_tax_config_backfill_map m
        WHERE m.namespace = p.namespace
          AND m.stripe_code = p.stripe_code
    )
    RETURNING id, namespace, app_mappings
)
INSERT INTO _addon_tax_config_backfill_map (namespace, stripe_code, winner_id, created)
...
BEGIN
  ...
  IF tax_code_mismatches > 0 OR behavior_mismatches > 0 THEN
    RAISE EXCEPTION 'add-on tax config backfill: addon_rate_cards still has % row(s) with inconsistent tax code representations and % row(s) with inconsistent tax behavior representations', tax_code_mismatches, behavior_mismatches;
  END IF;
END $$;
...
ALTER TABLE "addon_rate_cards"
  ADD CONSTRAINT "addon_rate_card_tax_behavior_consistency"
    CHECK (tax_behavior IS NOT DISTINCT FROM tax_config ->> 'behavior'),
  ADD CONSTRAINT "addon_rate_card_tax_code_consistency"
    CHECK (
...
DECLARE
  ...
  created_count int;
BEGIN
  SELECT count(*) INTO attached FROM _addon_tax_config_backfill_map WHERE NOT created;
  SELECT count(*) INTO created_count FROM _addon_tax_config_backfill_map WHERE created;
  RAISE NOTICE 'add-on tax config backfill: resolved % (namespace, code) pair(s); % attached to existing tax codes, % created new tax codes', attached + created_count, attached, created_count;
END $$;

-- Session-scoped temp tables are dropped explicitly because Atlas dry-run
-- validation cannot model ON COMMIT DROP.
DROP TABLE _addon_tax_config_backfill_pairs;
DROP TABLE _addon_tax_config_backfill_map;
