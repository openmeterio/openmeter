...
-- JSON tax config on subscription items. A NULL tax_config is valid and is left
-- untouched.
--
...
-- still allowing subscription items with no tax_config, behavior-only tax configs, and
...
BEGIN
  ...
  IF tax_code_conflicts > 0 OR behavior_conflicts > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % row(s) have conflicting tax_code_id representations and % row(s) have conflicting tax behavior representations; repair them before re-running', tax_code_conflicts, behavior_conflicts;
  END IF;
END $$;
...
BEGIN
  ...
  IF unresolved_count > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % row(s) contain an embedded tax_code_id that cannot be resolved to a live tax code in the same namespace', unresolved_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF invalid_reference_count > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % row(s) reference a missing, deleted, or cross-namespace tax code', invalid_reference_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF mismatched_count > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % row(s) contain a Stripe code that does not match the referenced live tax code Stripe app mapping', mismatched_count;
  END IF;
END $$;

-- Collect only the (namespace, Stripe code) pairs used by subscription items that
-- still need a normalized tax code reference.
DROP TABLE IF EXISTS _subscription_tax_config_backfill_pairs;
CREATE TEMP TABLE _subscription_tax_config_backfill_pairs AS
SELECT DISTINCT
    r.namespace,
...
CREATE INDEX ON _subscription_tax_config_backfill_pairs (namespace, stripe_code);
...
BEGIN
  ...
  FROM _subscription_tax_config_backfill_pairs
  WHERE stripe_code !~ '^txcd_[0-9]{8}$';

  IF invalid_count > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % (namespace, code) pair(s) carry a non-Stripe tax code; correct or clear these tax configs and re-run. Offending codes: %', invalid_count, invalid_codes;
  END IF;
END $$;

-- Resolve each pair to the entity selected by the read path's deterministic
-- tie-break.
DROP TABLE IF EXISTS _subscription_tax_config_backfill_map;
CREATE TEMP TABLE _subscription_tax_config_backfill_map (
...
INSERT INTO _subscription_tax_config_backfill_map (namespace, stripe_code, winner_id)
WITH expanded AS (
    SELECT
        t.id,
        t.namespace,
...
ranked AS (
    ...
    FROM expanded e
    JOIN _subscription_tax_config_backfill_pairs p
...
BEGIN
  ...
  FROM _subscription_tax_config_backfill_pairs p
  JOIN tax_codes t
    ON t.namespace = p.namespace
   AND t.key = 'stripe_' || p.stripe_code
   AND t.deleted_at IS NULL
  LEFT JOIN _subscription_tax_config_backfill_map m
    ON m.namespace = p.namespace
   AND m.stripe_code = p.stripe_code
  WHERE m.winner_id IS NULL;

  IF orphaned_count > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: % orphaned auto-key(s); a live tax code entity holds the stripe_ key without the matching app mapping: %', orphaned_count, orphaned;
  END IF;
END $$;
...
WITH inserted AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM _subscription_tax_config_backfill_map m
        WHERE m.namespace = p.namespace
          AND m.stripe_code = p.stripe_code
    )
    RETURNING id, namespace, app_mappings
)
INSERT INTO _subscription_tax_config_backfill_map (namespace, stripe_code, winner_id, created)
SELECT i.namespace, i.app_mappings -> 0 ->> 'tax_code', i.id, TRUE
FROM inserted i;

-- Stamp both representations together. Multiple subscription items using the same pair
...
BEGIN
  ...
  IF tax_code_mismatches > 0 OR behavior_mismatches > 0 THEN
    RAISE EXCEPTION 'subscription tax config backfill: subscription_items still has % row(s) with inconsistent tax code representations and % row(s) with inconsistent tax behavior representations', tax_code_mismatches, behavior_mismatches;
  END IF;
END $$;
...
DECLARE
  ...
  created_count int;
BEGIN
  SELECT count(*) INTO attached FROM _subscription_tax_config_backfill_map WHERE NOT created;
  SELECT count(*) INTO created_count FROM _subscription_tax_config_backfill_map WHERE created;
  RAISE NOTICE 'subscription tax config backfill: resolved % (namespace, code) pair(s); % attached to existing tax codes, % created new tax codes', attached + created_count, attached, created_count;
END $$;

-- Session-scoped temp tables are dropped explicitly because Atlas dry-run
-- validation cannot model ON COMMIT DROP.
DROP TABLE _subscription_tax_config_backfill_pairs;
DROP TABLE _subscription_tax_config_backfill_map;
