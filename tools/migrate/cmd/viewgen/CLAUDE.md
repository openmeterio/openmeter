...
**stdlib flag for single-purpose tools** — Uses the stdlib flag package (not Cobra/Viper) for the two flags --schema and --out. New flags must use flag.String with the same pattern. (`schemaPath = flag.String("schema", "./openmeter/ent/schema", "...")`)
