...
**Nil guard for optional features** — Check internal.App.ChargesAutoAdvancer == nil at the start of every RunE and return a descriptive error; the field is nil when charges are disabled in config. (`if internal.App.ChargesAutoAdvancer == nil { return fmt.Errorf("charges are not enabled") }`)
...
| File | Role | Watch For |
|------|------|-----------|
| `advancecharges.go` | Defines parent Cmd plus ListCmd, CustomerCmd, AllCmd for charge advancement. CustomerCmd requires --namespace and exactly one CUSTOMER_ID arg. | Every RunE must check ChargesAutoAdvancer != nil first — the field is nil when billing charges are disabled, and a missing guard panics with no user-friendly message. |

## Anti-Patterns

- Omitting the nil check on ChargesAutoAdvancer — panics when the charges feature is disabled
...
- **Explicit nil check for the optional ChargesAutoAdvancer field at every RunE entry** — Charges are conditionally wired; Wire produces a nil pointer when disabled. The guard yields a clear user-facing error instead of a runtime panic.

## Example: Adding a charge sub-command that requires the optional ChargesAutoAdvancer
