...
**Sub-command factory functions** — Each sub-command is a `var <Name>Cmd = func() *cobra.Command { ... }` factory rather than a top-level var, allowing deferred flag registration and avoiding init-order issues. (`var ListCmd = func() *cobra.Command { cmd := &cobra.Command{Use: "list", ...}; return cmd }`)
...
**Nil slice for optional namespace filter** — The namespace filter is a []string built conditionally so the service receives nil (unfiltered) when no namespace is specified, not an empty slice. (`var ns []string; if namespace != "" { ns = append(ns, namespace) }`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `advance.go` | Defines the parent Cmd plus ListCmd, InvoiceCmd, and AllCmd factory functions for invoice advance operations. | InvoiceCmd validates namespace is non-empty before calling AdvanceInvoice; replicate this guard on any new namespace-scoped sub-command. AllCmd registers its own --batch flag on cmd.PersistentFlags(), not the parent. |

## Anti-Patterns

- Constructing billing services locally instead of using internal.App
- Using context.Background() instead of cmd.Context()
- Registering sub-command-specific flags on the parent Cmd (pollutes sibling commands)
- Hardcoding namespace strings instead of accepting them via the --namespace flag
- Calling AdvanceInvoice without first validating that namespace is non-empty

## Decisions

- **Thin Cobra wrappers delegate to internal.App service fields.** — Keeps cmd/* free of business logic; all wiring happens in app/common via Wire so each sub-command is independently testable and the DI graph is compile-time verified.

## Example: Adding a new advance sub-command scoped to a customer
...
var CustomerCmd = func() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if namespace == "" { return fmt.Errorf("namespace is required") }
			_, err := internal.App.BillingAutoAdvancer.AdvanceInvoice(cmd.Context(), billing.InvoiceID{Namespace: namespace, ID: args[0]})
			return err
		},
	}
	return cmd
