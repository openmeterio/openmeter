...
> Binary entrypoint for the main HTTP API server: wires all domain services via Wire, runs DB migrations, registers namespace handlers in strict order (Ledger then KafkaIngest before initNamespace), provisions sandbox app and default billing profile, constructs router.Config with all services, and runs an oklog/run group with API server, telemetry server, Kafka producer, and notification event handler.
...
**Largest Application struct in codebase** — cmd/server Application exposes ~40 fields covering every domain service (Addon, AppRegistry, BillingRegistry, CostService, CreditGrantService, Ledger, EntitlementRegistry, KafkaProducer, NotificationEventHandler, etc.) — all declared in wire.go. (`type Application struct { common.GlobalInitializer; common.Migrator; Addon addon.Service; AppRegistry common.AppRegistry; BillingRegistry common.BillingRegistry; ... }`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `main.go` | Orchestrates full startup: config parse → Wire init → SetGlobals → Migrate → namespace handler registration → initNamespace → provisioning → server construction → run group. | Any new namespace.Handler must be registered BEFORE initNamespace(). Any new post-migration provisioning step must happen before the run.Group starts. |
| `wire.go` | Largest Wire file: lists ~30 provider sets including common.App, common.Billing, common.LedgerStack, common.Kafka, common.KafkaIngest, common.Server. Add new service dependencies here. | common.LedgerStack is the credits-aware ledger provider — don't construct ledger services outside it. common.WatermillNoPublisher (not common.Watermill) is used because server publishes but does not consume Kafka events. |
| `wire_gen.go` | Generated — DO NOT EDIT. The longest generated file (~600 lines of constructor calls). Reference to understand dependency order. | credits-guarded ledger construction appears here — verify noop vs real implementation when debugging credits.enabled=false paths. |

## Anti-Patterns

- Registering a namespace.Handler after initNamespace — the default namespace will be missing that handler's provisioning
...
## Example: Adding a new domain service to cmd/server and its HTTP handler
