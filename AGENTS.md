...
Use `Taskfile.yml` for all common tasks. `Makefile` files are compatibility shims.
...
Development commands are run via `Taskfile.yml`; it contains all commonly used commands during development. Use `task` commands for common tasks like running tests, generating code, linting, etc.
...
The root `Taskfile.yml` is the public entrypoint and should stay small: include domain taskfiles from `tools/tasks/*.yml` with `flatten: true`, plus shared internal helpers. Keep task names stable at the root so local commands, CI, and Makefile shims can all call the same surface.

Put new task definitions in the most relevant `tools/tasks/*.yml` file (`api.yml`, `build.yml`, `dev.yml`, `e2e.yml`, `lint.yml`, `release.yml`, or `test.yml`). Prefer Taskfile features such as `preconditions`, `prompt`, `defer`, `summary`, aliases, and sequential `task:` calls over ad hoc migration helper scripts. Keep standalone shell scripts only when they are reusable project tools with a clear owner, such as package-specific release scripts or existing migration utilities.
...
| Task | Command |
|------|---------|
| Start dependencies | `task up` |
| Stop dependencies | `task down` |
| Run API server (hot reload) | `task server` |
| Run all tests | `task test` |
| Run e2e tests | `task e2e` |
| Generate all code | `task generate-all` |
| Generate Go code only | `task generate` (runs `go generate ./...`) |
| Generate API + SDKs | `task gen-api` |
| Lint all | `task lint` |
| Lint Go only | `task lint-go` |
| Format code | `task fmt` |
| Tidy modules | `task mod` |
| Build all binaries | `task build` |
...
Task dependencies (`deps`) run in parallel. When a task must run after another task's generated output is fresh, call the dependency sequentially from `cmds` instead of listing both under `deps`.


| Generated artifact | Source | Regenerate with |
|---|---|---|
| `api/openapi.yaml`, `api/openapi.cloud.yaml` | TypeSpec in `api/spec/` | `task gen-api` |
| `api/client/javascript/`, `api/client/go/` | OpenAPI spec | `task gen-api` |
| `api/api.gen.go`, `api/v3/api.gen.go` | OpenAPI spec via oapi-codegen | `task gen-api` |
| `api/client/go/client.gen.go` | OpenAPI spec | `task gen-api` |
| `**/ent/db/` | Ent schema in `openmeter/ent/schema/` | `task generate` |
| `**/wire_gen.go` | Wire providers in `**/wire.go` | `task generate` |
| `**/convert.gen.go` | Goverter converter interfaces (`**/convert.go`) | `task generate` |
| `billing/derived.gen.go` | Goderive annotations | `task generate` |
...
2. Run `task gen-api` to regenerate OpenAPI spec and SDKs
3. Run `task generate` to regenerate Go server/client code
...
2. Run `task generate` (or `go generate ./...`)
...
2. Run `task generate` to regenerate ent code in `openmeter/ent/db/`
...
nix develop --impure .#ci -c task lint-go
nix develop --impure .#ci -c env POSTGRES_HOST=127.0.0.1 go test -tags=dynamic ./openmeter/ledger/historical/...
```

| Command | Description |
|---------|-------------|
| `task test` | Run all tests (parallel: `-p 128 -parallel 16`) |
| `task test-nocache` | Run tests bypassing cache |
| `task test-all` | Run tests including Svix/Redis dependencies |
| `task e2e` | Run e2e tests (requires docker compose dependencies) |
...
- Copy `config.example.yaml` to `config.yaml` (done automatically by Taskfile service targets and Makefile shims)
...
- Taskfile service targets will warn if `config.yaml` is outdated vs `config.example.yaml`
