...
Keep all schema changes in a pull request in one migration pair. If a follow-up
schema change is discovered before merge, fold it into that pull request's
existing migration and regenerate `atlas.sum` instead of adding another
migration.
...
Do not run multiple `nix develop` invocations concurrently in the same worktree. Their `devenv:files:cleanup` and `devenv:enterShell` tasks share `.devenv/` state, so one invocation can remove files such as `.devenv/load-exports` while another is using them. Run Nix-shell verification commands sequentially.
