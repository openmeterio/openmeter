...
Use `just` for common development tasks. The `Makefile` is a compatibility shim that forwards old targets to `just`.
...
Development commands are run via `justfile`; it contains the commonly used commands for tests, generation, linting, builds, and local services. Existing `make` targets remain as compatibility shims, but prefer `just` in new docs and workflows.
...
| Task | Command |
|------|---------|
| Start dependencies | `just up` |
| Stop dependencies | `just down` |
| Run API server (hot reload) | `just server` |
| Run all tests | `just test` |
| Run e2e tests | `just e2e` |
| Generate all code | `just generate-all` |
| Generate Go code only | `just generate` (runs `go generate ./...`) |
| Generate API + SDKs | `just gen-api` |
| Lint all | `just lint` |
| Lint Go only | `just lint-go` |
| Format code | `just fmt` |
| Tidy modules | `just mod` |
...
|---|---|---|
| `api/openapi.yaml`, `api/openapi.cloud.yaml` | TypeSpec in `api/spec/` | `just gen-api` |
| `api/client/javascript/`, `api/client/go/` | OpenAPI spec | `just gen-api` |
| `api/api.gen.go`, `api/v3/api.gen.go` | OpenAPI spec via oapi-codegen | `just gen-api` |
| `api/client/go/client.gen.go` | OpenAPI spec | `just gen-api` |
| `**/ent/db/` | Ent schema in `openmeter/ent/schema/` | `just generate` |
| `**/wire_gen.go` | Wire providers in `**/wire.go` | `just generate` |
| `**/convert.gen.go` | Goverter converter interfaces (`**/convert.go`) | `just generate` |
| `billing/derived.gen.go` | Goderive annotations | `just generate` |
...
2. Run `just gen-api` to regenerate OpenAPI spec and SDKs
3. Run `just generate` to regenerate Go server/client code
...
2. Run `just generate` (or `go generate ./...`)
...
2. Run `just generate` to regenerate ent code in `openmeter/ent/db/`
...
nix develop --impure .#ci -c just lint-go
nix develop --impure .#ci -c env POSTGRES_HOST=127.0.0.1 go test -tags=dynamic ./openmeter/ledger/historical/...
```

| Command | Description |
|---------|-------------|
| `just test` | Run all tests (parallel: `-p 128 -parallel 16`) |
| `just test-nocache` | Run tests bypassing cache |
| `just test-all` | Run tests including Svix/Redis dependencies |
| `just e2e` | Run e2e tests (requires docker compose dependencies) |
...
Key flags: `-tags=dynamic` (required for confluent-kafka-go), `-p 128 -parallel 16` (used by `just test`). Set `POSTGRES_HOST=127.0.0.1` or tests requiring Postgres will be skipped.
...
just build-server       # Just the server
```
...
- Copy `config.example.yaml` to `config.yaml` (done automatically by service runner tasks)
...
- Service runner tasks will warn if `config.yaml` is outdated vs `config.example.yaml`
