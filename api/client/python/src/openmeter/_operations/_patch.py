...
def build_query_meter_request(
    ...
    **kwargs: Any,
    ...
    if from_parameter is not None:
        _params["from"] = _SERIALIZER.query(
            "from_parameter", from_parameter, "iso-8601"
        )
    if to is not None:
    ...
    if window_time_zone is not None:
        _params["windowTimeZone"] = _SERIALIZER.query(
            "window_time_zone", window_time_zone, "str"
        )
    if subject is not None:
        _params["subject"] = _SERIALIZER.query("subject", subject, "[str]")
    if filter_group_by is not None:
        for key in filter_group_by:
            _params["filterGroupBy[{}]".format(key)] = _SERIALIZER.query(
                "filter_group_by", filter_group_by[key], "str"
            )
