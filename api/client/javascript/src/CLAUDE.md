...
**Cross-cutting: transformResponse wraps all fetch calls** — Every public method in src/client/ must pipe the raw fetch result through transformResponse (handles date decoding and error mapping). Portal client reuses the same utility from src/client/utils.js. (`// client/events.ts: return transformResponse(await this.client.GET('/api/v1/events', {...}))`)
**RequestOptions = Pick<RequestInit, 'signal'> threaded through** — Every public SDK method accepts options?: RequestOptions and spreads ...options into the fetch call to propagate abort signals. (`// client/events.ts: async list(params, options?: RequestOptions) { ...options }`)
...
- Adding admin-level endpoints to src/portal/ — the portal client is intentionally restricted
