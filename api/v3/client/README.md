...
| `om.Entitlements.List` | `GET /openmeter/entitlements` | List the entitlements of every customer in the namespace that are active at the time of the request. Intended for administrative use; to list the entitlements of a single customer, use the customer entitlements endpoints, and for checking entitlement access, use the entitlement access endpoints instead. |
| `om.Entitlements.Get` | `GET /openmeter/entitlements/{entitlementId}` | Get an entitlement by ID. For checking entitlement access, use the entitlement access endpoints instead. |
