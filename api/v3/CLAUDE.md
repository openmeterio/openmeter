...
> Infrastructure and generated code layer for the v3 HTTP API: api.gen.go is generated from openapi.yaml via oapi-codegen and must never be hand-edited; all business logic lives in sub-packages (handlers/, server/, apierrors/, filters/, etc.). Primary constraint: api/v3 itself is purely infrastructure — no domain calls, no business logic.

## Patterns

**Generated stub is the sole ServerInterface source** — api.gen.go is generated from openapi.yaml via the //go:generate directive in api.go. Never edit api.gen.go directly — all changes start in api/spec/ then run make gen-api. (`//go:generate go tool github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen --config=codegen.yaml ./openapi.yaml`)
...
**always-prefix-enum-values** — codegen.yaml sets always-prefix-enum-values: true, so all enum constants are prefixed with the type name. Always use the fully prefixed constant in handler code. (`BillingAppTypeSandbox BillingAppType = "sandbox" // not just Sandbox`)
...
**All error responses via apierrors package** — v3 handlers must use apierrors named constructors and route through GenericErrorEncoder. Direct w.WriteHeader or http.Error calls are forbidden. (`return apierrors.NewBadRequestProblemResponse(ctx, err)`)
**OAS request validation via oasmiddleware.ValidateRequest** — The Chi router in api/v3/server mounts oasmiddleware.ValidateRequest before the generated server handler. Build once at startup — NewValidationRouter is expensive. (`router.Use(oasmiddleware.ValidateRequest(spec, hooks...))`)
