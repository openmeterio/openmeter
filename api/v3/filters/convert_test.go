...
func TestConvertFilterDateTime(t *testing.T) {
	t.Run("eq maps to filter.FilterTime Eq", func(t *testing.T) {
		out, err := FromAPIFilterDateTime(&FilterDateTime{
	...
	t.Run("single-bound gt maps directly", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), *out.Gt)
		assert.Nil(t, out.And)
	})
	...
	t.Run("gt and lt are split into an And", func(t *testing.T) {
		out, err := FromAPIFilterDateTime(&FilterDateTime{
			Gt: lo.ToPtr("2024-01-01T00:00:00Z"),
			Lt: lo.ToPtr("2024-12-31T23:59:59Z"),
		})
		require.NoError(t, err)
		assert.Nil(t, out.Gt, "top-level Gt must be empty when range is split")
		...
		require.NotNil(t, out.And)
		require.Len(t, *out.And, 2)
		assert.Equal(t, time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), *(*out.And)[0].Gt)
		assert.Equal(t, time.Date(2024, 12, 31, 23, 59, 59, 0, time.UTC), *(*out.And)[1].Lt)
	})
	...
	t.Run("gte and lte are split into an And", func(t *testing.T) {
		...
		require.NotNil(t, out.And)
		require.Len(t, *out.And, 2)
		assert.Equal(t, time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), *(*out.And)[0].Gte)
		assert.Equal(t, time.Date(2024, 12, 31, 23, 59, 59, 0, time.UTC), *(*out.And)[1].Lte)
	})

	t.Run("invalid timestamp per op returns error with op name", func(t *testing.T) {
		cases := map[string]FilterDateTime{
			"eq":  {Eq: lo.ToPtr("not-a-date")},
			"gt":  {Gt: lo.ToPtr("not-a-date")},
			"gte": {Gte: lo.ToPtr("not-a-date")},
			"lt":  {Lt: lo.ToPtr("not-a-date")},
			"lte": {Lte: lo.ToPtr("not-a-date")},
		}
		for op, in := range cases {
			t.Run(op, func(t *testing.T) {
				_, err := FromAPIFilterDateTime(&in)
				require.Error(t, err)
				assert.Contains(t, err.Error(), "filter "+op)
			})
		}
	})
