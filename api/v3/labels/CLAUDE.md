...
> Bridges the api.Labels wire type (map[string]string) to the domain models.Metadata and models.Annotations types, enforcing key/value format rules and the 'openmeter_' annotation prefix convention. All v3 endpoints that accept or return labels must route through this package.
...
**FromMetadataAnnotations for outbound label conversion** — When building a v3 response from a domain entity, call labels.FromMetadataAnnotations(entity.Metadata, entity.Annotations) to produce *api.Labels. Annotation keys are re-prefixed with 'openmeter_'; invalid keys are silently skipped. (`resp.Labels = labels.FromMetadataAnnotations(entity.Metadata, entity.Annotations)`)
**ValidateLabel / ValidateLabels for pre-write validation** — Call ValidateLabel(k, v) or ValidateLabels(labels) to check format (^[a-zA-Z0-9][...]{1,63}$) and reserved prefixes (_,kong,konnect,insomnia,mesh,kic,kuma,openmeter). Returns models.ValidationIssue errors with http.StatusBadRequest attribute so they propagate through the error encoder. (`if err := labels.ValidateLabels(*req.Labels); err != nil { return err }`)
...
```
import (
    apiLabels "github.com/openmeterio/openmeter/api/v3/labels"
)
