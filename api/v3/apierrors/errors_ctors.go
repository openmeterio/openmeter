...
const (
	...
	// BadRequest
	// NOTE: there is not `detail` attribute in a 400.
	BadRequestType  = "https://kongapi.info/konnect/bad-request"
	BadRequestTitle = "Bad Request"

	// BadReqeust UUID Invalid Format
	InvalidUUIDFormat = "invalid ID format"
	...
	ConflictTitle = "Resource Conflict"
...
func NewBadRequestInvalidUUIDError(ctx context.Context, err error, field string) *BaseAPIError {
	return NewBadRequestError(ctx, err, InvalidParameters{
		{
			Field:  field,
			Reason: InvalidUUIDFormat,
		},
	})
}
