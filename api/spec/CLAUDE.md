...
**make gen-api is the only regeneration path** — Running pnpm generate → pnpm --filter compile → openapi bundle → cp produces all output artefacts. Direct tsc or tsp compile calls are not the canonical path. (`Makefile generate target: pnpm --filter @openmeter/api-spec-aip run compile → openapi bundle → ../../../v3/openapi.yaml`)
...
**New legacy sub-domain file must be registered in both main.tsp and cloud/main.tsp** — packages/legacy/src/main.tsp and packages/legacy/src/cloud/main.tsp are the compilation entry points; files not imported there are silently ignored. (`Adding packages/legacy/src/myfeature.tsp: add `import './myfeature.tsp';` to both main.tsp files.`)
