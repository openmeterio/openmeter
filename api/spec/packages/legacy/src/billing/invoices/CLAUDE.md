...
**GOBL-derived schema with explicit omission comments** — Models mirror GOBL schema but deliberately omit unsupported fields. Each omission must be documented in a comment block explaining why (e.g. '/* Omitted: multi-currency exchange_rates — not supported yet */') so reviewers know the gap is intentional. (`/* Omitted fields: ordering, delivery — physical goods not supported */`)
