...
**@friendlyName required — error severity** — `friendlyNameRule` is severity `error` (not warning) and checks that every named interface, model, enum, and union has a decorator whose `decorator.name === '$friendlyName'`. Violations block compilation. (`const hasFriendlyName = node.decorators.some((d) => d.decorator.name === '$friendlyName')`)
**Doc decorator required on models and properties** — `docDecoratorRule` calls `getDoc(context.program, target)` to check presence of `@doc`. Models named with `*Response` suffix are exempt from property-level doc checks. `_` and `contentType` properties are exempt. (`if (target.name && !getDoc(context.program, target)) { context.reportDiagnostic(...) }`)
**Composition over inheritance enforcement** — `compositionOverInheritanceRule` warns when a model extends another model (via `model.node.extends`) without a `@discriminator` on the base. Uses `SyntaxKind.ModelStatement` from `@typespec/compiler/ast` — import path is `/ast`, not root. (`if (model.baseModel && model.node?.kind === SyntaxKind.ModelStatement && model.node.extends && getDiscriminator(context.program, model.baseModel) === undefined) { ... }`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `utils.js` | Exports `isPascalCaseNoAcronyms`, `isCamelCaseNoAcronyms`, `isSnakeCase`, `isKebabCase`. The Pascal regex has hardcoded exceptions: `['OAuth2', 'URL', 'API', 'UI', 'ID']`. | Adding new acronym exceptions requires updating `pascalCaseExceptions` in utils.js; the regex is derived from that array at runtime. Never duplicate casing logic in individual rule files. |
| `casing.js` | Enforces PascalCase on model names and camelCase on property names. | The dual condition `(name !== '_' && !isCamelCase(name)) || isPascalCase(name)` means a property that accidentally becomes PascalCase is caught even if the first check passes. |
| `friendly-name.js` | Error-severity rule requiring `@friendlyName` on every named model, enum, union, and interface. | Severity is `error`, not `warning` — violations block compilation, not just lint. Do not downgrade to warning. |
...
| `composition-over-inheritance.js` | Detects model inheritance without `@discriminator` on the base; distinguishes template-instance inheritance from plain inheritance for message selection. | Uses `SyntaxKind.ModelStatement` from `@typespec/compiler/ast` — import path is `/ast`, not the root package. |
| `operation-summary.js` | Warns when an operation lacks a `@summary` decorator. | Checks for `$summary` (dollar-prefixed internal decorator name), not the user-facing `@summary` string. |
...
- Changing `friendlyNameRule` severity from `error` to `warning` — it is intentionally blocking because missing `@friendlyName` breaks SDK code generation.
