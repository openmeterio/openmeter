...
| File | Role | Watch For |
|------|------|-----------|
| `CLAUDE.md` | Archie intent layer for the common/ organisational folder; describes the role of definitions/ and the centralisation decision. | Do not add TypeSpec source or OpenAPI YAML fragments directly here — all component fragments belong in definitions/. |
...
- **All shared components live in a single definitions/ child rather than scattered alongside route files** — Centralising $ref targets prevents duplicate schema definitions and ensures the TypeSpec build pipeline resolves all component references from one canonical location.
