...
> TypeSpec definitions for the Features domain: feature models (meter-backed with optional unit costs), cost query types, and CRUD+query operations compiled to v3 OpenAPI and Go/JS/Python SDKs. All types live under namespace Features; and operations are split across two interfaces (FeatureOperations and FeatureCostOperations).
...
**Extension markers for private/unstable/internal endpoints** — Operations not yet public must carry the appropriate extension markers. All current feature operations carry @extension(Shared.UnstableExtension, true) and @extension(Shared.InternalExtension, true). (`@extension(Shared.UnstableExtension, true)
...
| File | Role | Watch For |
|------|------|-----------|
| `operations.tsp` | Declares FeatureOperations and FeatureCostOperations interfaces with all HTTP verbs, route params, and response types. | Must import @typespec/http and use `using TypeSpec.Http;` — omitting this breaks @query/@get/@post decorators. deepObject+explode style required for filter params. |
...
| `feature.tsp` | Core Feature model using Shared.ResourceWithKey spread and optional meter + unit_cost fields. | FeatureUpdateRequest only includes updatable fields (unit_cost); do not add read-only fields like id or created_at to it. |
| `cost.tsp` | FeatureCostQueryRow and FeatureCostQueryResult models for the cost query endpoint; reuses Meters.MeterQueryRequest as request body. | cost field is nullable (Shared.Numeric | null) — requires #suppress on no-nullable rule. The detail field explains why cost is null. |
...
- **LLM unit cost uses a flat model with mutually exclusive static/property fields rather than a nested union** — Keeps the API surface flat and JSON-friendly; TypeSpec discriminated unions cannot enforce mutual exclusion so the constraint is documented and validated at the Go service layer.
