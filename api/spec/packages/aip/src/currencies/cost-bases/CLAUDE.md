...
**Extension decorators for stability** — Every operation must carry @extension(Shared.UnstableExtension, true) and @extension(Shared.InternalExtension, true). Missing these breaks internal API maturity tracking. (`@extension(Shared.UnstableExtension, true) @extension(Shared.InternalExtension, true) @get`)
...
| File | Role | Watch For |
|------|------|-----------|
| `cost-basis.tsp` | Defines the CostBasis model — single source of truth for field names, types, and visibility. @friendlyName sets the generated Go/JS type name. | Omitting @visibility causes fields to appear in both create and read payloads. @friendlyName must be globally unique across the spec — duplicates cause SDK type collisions. |
...
- **All operations marked @extension(Shared.InternalExtension) and @extension(Shared.UnstableExtension)** — Cost-basis endpoints are billing-internal and not yet stable; the extensions keep them out of public SDK docs and signal breaking-change freedom.
