...
> TypeSpec definitions for the LLM cost pricing domain: global synced price models, per-namespace override CRUD, and filter/pagination operations compiled to v3 OpenAPI.
...
**Read-only visibility for all Price response fields** — The Price model fields all use `@visibility(Lifecycle.Read)` since prices are system-managed; OverrideCreate is a separate flat input model for writes. (`@visibility(Lifecycle.Read)
source: PriceSource;`)
...
- Using @visibility(Lifecycle.Create/Update) on Price model fields — prices are read-only responses; OverrideCreate is the write input
