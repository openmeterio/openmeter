...
function stringConstraints($: Typekit, type: Scalar, member?: ModelProperty) {
  ...
  for (const [name, value] of Object.entries(constraints)) {
    ...
    if (name === 'minLength' && value !== 0) {
      ...
      parts.push(callPart('regex', `new RegExp(${JSON.stringify(value)})`))
    } else if (name === 'format') {
...
function maxNumeric<T extends number | bigint>(
  ...
  return definedValues.reduce(
    (max, current) => (current > (max ?? -Infinity) ? current : max),
    definedValues[0],
  )
}
...
function minNumeric<T extends number | bigint>(
  ...
  return definedValues.reduce(
    (min, current) => (current < (min ?? Infinity) ? current : min),
    definedValues[0],
  )
}
...
function numericConstraintsToParts(
  ...
  for (const [name, value] of Object.entries(constraints)) {
    ...
    const literal =
      useBigInt || typeof value === 'bigint'
        ? `${BigInt(value as number | bigint)}n`
        : `${value}`
...
function zodStringFormatMethod(format: string): string | undefined {
  switch (format) {
    // `uri`/`url` intentionally fall through to plain string: Zod 4 deprecated
    // the chained `.url()` validator, and we don't diverge the schema for it.
    case 'email':
    case 'uuid':
    case 'ulid':
    case 'cuid':
