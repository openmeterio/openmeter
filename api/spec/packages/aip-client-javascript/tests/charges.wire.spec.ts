...
// introduced actually land: `invoice` is
// `ChargeRealizationInvoice | ChargeRealizationInvoiceReference`, where the
// expanded side is the charges API's own invoice-header model (a wide object
// that must not collapse onto the id-only reference). `detailed_lines` (the
// `realization.detailed_lines` expand) is an array of a discriminated union.
//
...
describe('charge realization wire mapping', () => {
  it('keeps the expanded invoice instead of collapsing it onto the reference', () => {
    const expanded = asRecord(sampleSnake(chargeRealizationInvoice))
  ...
  it('maps the invoice reference, the variant the operation sweep never samples', () => {
    const wire = realizationWith(sampleSnake(chargeRealizationInvoiceReference))
  ...
  it('leaks no wire casing across a fully expanded realization', () => {
    const wire = realizationWith(sampleSnake(chargeRealizationInvoice))
