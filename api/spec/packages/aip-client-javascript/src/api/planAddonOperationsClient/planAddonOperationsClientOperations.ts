...
} from "./planAddonOperationsClientContext.js";
...
async function listPlanAddonsSend(
  client: PlanAddonOperationsClientContext,
  planId: string,
  options?: Record<string, any>,
) {
  const path = parse("/{planId}{?page*}").expand({
    planId: planId,
    ...(options?.page && {page: {
 ...
 *
 * @param {PlanAddonOperationsClientContext} client
 * @param {string} planId
...
export async function getPlanAddon(
  ...
  const path = parse("/{planAddonId}/{planId}").expand({
    planId: planId,
