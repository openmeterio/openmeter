...
} from "./eventsEndpointsClientContext.js";
...
async function listSend(
  client: EventsEndpointsClientContext,
  options?: Record<string, any>,
) {
  const path = parse("/openmeter/events{?page*,filter*,sort}").expand({
 ...
 *
 * @param {EventsEndpointsClientContext} client
...
export async function ingestEvent(
  ...
  const path = parse("/openmeter/events").expand({});
  const httpRequestOptions = {
    headers: {
