...
import {
  CustomerBillingEndpointsClientContext,
} from "./customerBillingEndpointsClientContext.js";
...
export async function get(
  client: CustomerBillingEndpointsClientContext,
  customerId: string,
  options?: GetOptions,
): Promise<CustomerBillingData | void> {
  const path = parse("/openmeter/customers/{customerId}/billing").expand({
...
export async function upsertAppData(
  ...
  const path = parse("/openmeter/customers/{customerId}/billing/app-data").expand({
    customerId: customerId
  });
  const httpRequestOptions = {
 ...
 * application.
 *
 * @param {CustomerBillingEndpointsClientContext} client
...
export async function createCheckoutSession(
  ...
  const path = parse("/openmeter/customers/{customerId}/billing/stripe/checkout-sessions").expand({
    customerId: customerId
  });
  const httpRequestOptions = {
    headers: {
...
export async function createPortalSession(
  ...
  const path = parse("/openmeter/customers/{customerId}/billing/stripe/portal-sessions").expand({
    customerId: customerId
  });
  const httpRequestOptions = {
    headers: {
