...
import type {
  TaxCodesEndpointsClientContext,
} from "./taxCodesEndpointsClientContext.js";
...
export async function create(
  client: TaxCodesEndpointsClientContext,
  taxCode: CreateRequest,
  options?: CreateOptions,
): Promise<TaxCode | void> {
  const path = parse("/openmeter/tax-codes").expand({});
...
export async function get(
  ...
  const path = parse("/openmeter/tax-codes/{taxCodeId}").expand({
    taxCodeId: taxCodeId
  });
  const httpRequestOptions = {
    headers: {},
...
async function listSend(
  ...
  const path = parse("/openmeter/tax-codes{?page*,include_deleted}").expand({
    ...(options?.page && {page: {
