...
import {
  CurrenciesCustomCostBasesOperationsClientContext,
} from "./currenciesCustomCostBasesOperationsClientContext.js";
...
async function getCostBasesSend(
  client: CurrenciesCustomCostBasesOperationsClientContext,
  currencyId: string,
  options?: Record<string, any>,
) {
  const path = parse("/{currencyId}{?filter*,page*}").expand({
 ...
 *
 * @param {CurrenciesCustomCostBasesOperationsClientContext} client
...
export async function createCostBasis(
  ...
  const path = parse("/{currencyId}").expand({
    currencyId: currencyId
  });
  const httpRequestOptions = {
