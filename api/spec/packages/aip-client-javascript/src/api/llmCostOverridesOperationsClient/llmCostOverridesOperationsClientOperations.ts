...
} from "./llmCostOverridesOperationsClientContext.js";
...
async function listOverridesSend(
  client: LlmCostOverridesOperationsClientContext,
  options?: Record<string, any>,
) {
  const path = parse("/{?filter*,page*}").expand({
 ...
 *
 * @param {LlmCostOverridesOperationsClientContext} client
...
export async function deleteOverride(
  ...
  const path = parse("/{priceId}").expand({
    priceId: priceId
  });
  const httpRequestOptions = {
    headers: {},
