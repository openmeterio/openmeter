...
test: ## Run tests
	...
	POSTGRES_HOST=127.0.0.1 go tool gotest.tools/gotestsum $(GOTESTSUM_FLAGS) -- $(GO_TEST_FLAGS) ./...

.PHONY: test-nocache
test-nocache: ## Run tests without cache
	$(call print-target)
	PGPASSWORD=postgres psql -h 127.0.0.1 -U postgres postgres -c "SELECT version();" || (echo "!!! Postgres is not running. Please start it with 'docker compose up -d postgres' !!!" && false)
	POSTGRES_HOST=127.0.0.1 go tool gotest.tools/gotestsum $(GOTESTSUM_FLAGS) -- $(GO_TEST_FLAGS) -count=1 ./...
...
test-all: ## Run tests with svix dependencies, bypassing the test cache
	...
	SVIX_HOST="localhost" SVIX_JWT_SECRET="$(SVIX_JWT_SECRET)" go tool gotest.tools/gotestsum $(GOTESTSUM_FLAGS) -- $(GO_TEST_FLAGS) -count=1 ./...

.PHONY: test-go-sdk
test-go-sdk: ## Build, vet, and test the v3 Go SDK module (api/v3/client)
	$(call print-target)
	go -C api/v3/client build ./...
	go -C api/v3/client vet ./...
	go -C api/v3/client test ./...
