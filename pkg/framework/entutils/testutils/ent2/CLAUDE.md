...
**Full feature set matching production** — FeatureVersionedMigration, FeatureLock, FeatureUpsert, FeatureExecQuery must all be present — identical to production and ent1 — to keep test-generated code consistent. (`Features: []gen.Feature{gen.FeatureVersionedMigration, gen.FeatureLock, gen.FeatureUpsert, gen.FeatureExecQuery}`)
...
**Package path scoped under testutils/ent2/db** — Target and Package must stay under pkg/framework/entutils/testutils/ent2/db to avoid overwriting ent1 or production generated code. (`Package: "github.com/openmeterio/openmeter/pkg/framework/entutils/testutils/ent2/db"`)

## Key Files

| File | Role | Watch For |
|------|------|-----------|
| `entc.go` | Codegen driver for the ent2 fixture; excluded from normal builds via build tag. | Adding entcursor or entpaginate here makes ent2 generate unnecessary code and blurs the distinction from ent1. |
| `generate.go` | //go:generate hook that runs entc.go for the ent2 fixture. | Package name must stay ent2; changing it breaks go generate resolution. |
...
## Example: Full entc.Generate call for ent2 with minimal extension (entexpose only)
...
func main() {
	err := entc.Generate("./schema", &gen.Config{
		Features: []gen.Feature{gen.FeatureVersionedMigration, gen.FeatureLock, gen.FeatureUpsert, gen.FeatureExecQuery},
// ...
```
