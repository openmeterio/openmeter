...
export async function createMeter(
  ...
  const body = {
    key:
      overrides.key ??
      `meter_${faker.string.alphanumeric({ length: 16, casing: "lower" })}`,
...
export async function createFeature(
  ...
  const body: Record<string, unknown> = {
    key:
      overrides.key ??
      `feature_${faker.string.alphanumeric({ length: 16, casing: "lower" })}`,
...
export async function ingestEvent(
  ...
  const res = await request.post(`${BASE}/events`, {
    headers: { "Content-Type": "application/cloudevents+json" },
    data: body,
  });
  expect(res.status(), `ingest failed: ${await res.text()}`).toBe(202);
}
