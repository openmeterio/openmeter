...
func TestV3Grants(t *testing.T) {
	...
	// anchor, and voids up to now, are inside the current usage period.
	...
	for i := range 3 {
		g, err := c.Customers.Entitlements.Grants.Create(t.Context(), cust.ID, ent.ID, v3sdk.EntitlementGrantCreateRequest{
			Amount:      "100",
			EffectiveAt: anchor.Add(time.Duration(i) * time.Minute),
		})
		c.requireStatus(http.StatusCreated, err)
	...
	t.Run("list filtered by customer and sorted descending", func(t *testing.T) {
		...
		require.True(t, res.Meta.Page.Next.IsNull())
	})

	t.Run("list page by page", func(t *testing.T) {
		var listed []string
		for g, err := range c.Grants.ListAll(t.Context(), v3sdk.ListGrantsParams{
			Page:   &v3sdk.CursorPageParams{Size: lo.ToPtr(1)},
			Filter: customerFilter,
		}) {
			require.NoError(t, err)
		...
		require.Equal(t, grantIDs, listed)
	})

	t.Run("list filtered by feature key", func(t *testing.T) {
		res, err := c.Grants.List(t.Context(), v3sdk.ListGrantsParams{
			Filter: &v3sdk.ListGrantsFilter{FeatureKey: &v3sdk.StringExactFilter{Eq: lo.ToPtr(f.Key)}},
		})
		c.requireStatus(http.StatusOK, err)
		require.Len(t, res.Data, 3)
	})

	t.Run("list sorted by a field that cannot key the cursor", func(t *testing.T) {
		_, err := c.Grants.List(t.Context(), v3sdk.ListGrantsParams{
			Sort: &v3sdk.Sort{By: "updated_at"},
		})
