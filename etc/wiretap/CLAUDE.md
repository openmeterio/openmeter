...
| File | Role | Watch For |
|------|------|-----------|
| `config.docker.yaml` | Wiretap proxy config: routes incoming requests on port 19090 to the local API server and serves the monitor UI on 19091/19092. The contract path is volume-mounted from the repo root. | If api/openapi.yaml is regenerated via 'make gen-api' and the volume mount path changes, update the contract field. If the local server port changes from 8888, update redirectURL. Avoid port conflicts with docker-compose.yaml services. |
