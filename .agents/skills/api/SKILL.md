...
Follow these API design standards based on [Kong AIP](https://kong-aip.netlify.app/list/).

### Resource & Field Naming (#122, #126)

- URL paths use kebab-case: `/api/v3/openmeter/llm-cost`
- Field names use `snake_case`: `primary_color`, `created_at`
- Enum values use `lowercase_snake_case`
- Enum zero/default value must be `unknown`
- Prefer enums over booleans — allows future expansion without breaking clients

### List Endpoints (#132)

- `GET` to entity root path (e.g., `GET /api/v3/openmeter/meters`)
- Response must contain a `data` key with an array of entities
- Must provide a default sort order (creation date, update date, or alphabetical — never UUID)
- Support `?sort=` query parameter: comma-delimited fields, `desc` suffix for descending (default ascending)
  - Example: `?sort=created_at desc,name asc`
  - JSONPath for nested: `foo.bar`
  - Invalid sort fields return `400`
- Sortable attributes must include all filterable attributes
- Trailing slash returns `404` (e.g., `GET /api/v3/openmeter/meters/` → 404)

### Pagination (#158)

- All collection endpoints must be paginated from inception
- **Cursor-based (preferred):** query via `page[size]`, `page[after]`, `page[before]` (after/before are mutually exclusive)
  - Cursors must be opaque (base64-encoded)
  - Response: `{ "data": [...], "meta": { "page": { "size": N, "next": <uri|null>, "previous": <uri|null> } } }`
- **Offset-limit:** query via `page[size]`, `page[number]` (defaults to 1)
  - Response: `{ "data": [...], "meta": { "page": { "number": N, "size": N, "total": N } } }`
- `total` (exact) or `estimated_total` (approximate) may be included at server discretion

### Filtering (#160)

- Syntax: `?filter[field]=value` (equality) or `?filter[field][operator]=value`
- Multiple filters use AND logic
- Operators: `eq` (default), `neq`, `oeq` (or-equal, comma-separated values), `contains`, `ocontains`, `lt`, `lte`, `gt`, `gte`
- List field quantifiers: `?filter[field][operator][any]=value` or `[all]`
- Label filtering via dot notation: `?filter[labels.key_name]=value`
- Unsupported filter fields return `400` with `invalid_parameters`

### Update Endpoints (#134)

- Both `PATCH` and `PUT` required for all entities
- **PATCH** — JSON Merge Patch (RFC 7396), operation ID: `update-<entity>`
  - Reject unknown fields with `400` (list in `invalid_parameters`)
  - Reject read-only fields with `400`
  - Null on non-required field → remove it; null on required+nullable → set to null; null on required+non-nullable → `400`
  - Nested objects: recursively merge, preserve unchanged siblings
  - Arrays: replace entirely (not merged)
  - Empty patch (`{}`) is a no-op
- **PUT** — Upsert, operation ID: `upsert-<entity>`, returns `201` (created) or `200` (replaced)

### Delete Endpoints (#135)

- No request body — all params in URL path
- Check permissions first → `403` if unauthorized (regardless of resource existence)
...
- All errors use RFC 7807 format with `application/problem+json` content type
- Required fields: `type` (unique error URL), `status` (HTTP code), `title` (sentence case), `detail` (context-specific), `instance` (correlation ID)
- **400 responses must include `invalid_parameters` array**, each entry with:
  - `field` (dot notation for body, param name for query), `reason` (human-readable), `source` (`path`|`body`|`header`|`query`), optional `rule`
- Validation rules: `required`, `min_length`, `max_length`, `min`, `max`, `enum` (with `choices`), `matches_regex`, `is_string`, `is_integer`, `is_uuid`, `unique`, etc.
- Use `404` (not `400`) for invalid URL path parameters
- Use `409 Conflict` for valid requests that fail due to server state (e.g., unique constraint)
- Return `404` instead of `403` for resources outside the user's organization (prevent data leakage)
...
- **Labels (#129):** `labels` field as `object` (key-value string pairs), max 50 entries, keys 1–63 chars (`[a-z0-9._-]`), values ≤255 chars. PATCH with `null` value deletes a label key.
- **Content-Type (#137):** Validate `Content-Type` header; return `415 Unsupported Media Type` for unsupported types. Default to `application/json` when no `Content-Type` is provided.
- **Bulk Delete (#235):** `POST /api/v3/openmeter/<entity>/bulk-delete` with `{ "ids": [...] }` body, max 100 items. Transactional (all-or-nothing) returns `204`; partial success returns `207 Multi-Status` with per-item results.
- **Versioning (#3101):** Version in URL path (e.g., `/api/v3/openmeter/`). Per-resource versioning, no minor versions.
- **Empty Fields (#3106):** Always return all fields in responses. Use `null` for absent scalars, `[]` for empty lists, `{}` for empty objects. Never omit fields.
